...
// prefers-reduced-motion: render one static frame instead of looping —
// per the project's own motion rule (respect reduced-motion, never just
// slow an animation down), not part of the upstream react-bits snippet.
...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    const tryStart = () => {
      // setSize() already rendered one static frame above — with reduced
      // motion, that's the whole animation, so never enter the rAF loop.

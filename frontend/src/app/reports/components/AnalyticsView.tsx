    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 bg-slate-900/60 p-4 rounded-xl border border-slate-800">
        <div className="flex items-center gap-3">
          <span className="text-xs font-semibold text-slate-300">Target Institution:</span>
          <select
            value={selectedCollegeId}
            onChange={(e) => onSelectCollege(e.target.value)}
            className="bg-slate-800 border border-slate-700 text-slate-200 text-xs px-3 py-1.5 rounded-lg focus:outline-none focus:border-blue-500 cursor-pointer min-w-[220px]"
          >
            <option value="all">🌐 All Partner Colleges (Consolidated)</option>
            {colleges.map((c) => (
      ...
      {kpi && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <div className="glass-card rounded-xl p-3.5 border border-slate-800">
            <p className="text-[11px] text-slate-400 font-medium">Total Calls Logged</p>
            <p className="text-xl font-black text-blue-400 mt-1 tabular-nums">{kpi.total_calls}</p>
            <p className="text-[10px] text-slate-500 mt-0.5">Daily Tracker Outreach</p>
          </div>

          {/* Card 2: Positive Rate */}
          <div className="glass-card rounded-xl p-3.5 border border-slate-800">
            <p className="text-[11px] text-slate-400 font-medium">Positive Outreach</p>
            <p className="text-xl font-black text-emerald-400 mt-1 tabular-nums">
              {kpi.positive_rate_pct}%
            </p>
            <p className="text-[10px] text-emerald-500/80 mt-0.5">{kpi.positive_responses} Opportunities</p>
          </div>

          {/* Card 3: Companies Contacted */}
          <div className="glass-card rounded-xl p-3.5 border border-slate-800">
            <p className="text-[11px] text-slate-400 font-medium">Companies Contacted</p>
            <p className="text-xl font-black text-purple-400 mt-1 tabular-nums">{kpi.companies_contacted}</p>
            <p className="text-[10px] text-purple-500/80 mt-0.5">Unique Corporate Leads</p>
          </div>

          {/* Card 4: JDs Received */}
          <div className="glass-card rounded-xl p-3.5 border border-slate-800">
            <p className="text-[11px] text-slate-400 font-medium">JDs Received</p>
            <p className="text-xl font-black text-cyan-400 mt-1 tabular-nums">{kpi.jd_received}</p>
            <p className="text-[10px] text-cyan-500/80 mt-0.5">Daily Leads Register</p>
          </div>

          {/* Card 5: Drives Completed */}
          <div className="glass-card rounded-xl p-3.5 border border-slate-800">
            <p className="text-[11px] text-slate-400 font-medium">Drives Completed</p>
            <p className="text-xl font-black text-amber-400 mt-1 tabular-nums">{kpi.drives_completed}</p>
            <p className="text-[10px] text-amber-500/80 mt-0.5">{kpi.drives_in_progress} In Progress</p>
          </div>

          {/* Card 6: Total Offers */}
          <div className="glass-card rounded-xl p-3.5 border border-slate-800">
            <p className="text-[11px] text-slate-400 font-medium">Total Student Offers</p>
            <p className="text-xl font-black text-emerald-400 mt-1 tabular-nums">{kpi.total_offers}</p>
            <p className="text-[10px] text-emerald-500/80 mt-0.5">2026 Batch Placements</p>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3.5 space-y-2">
              <span className="font-bold text-blue-400 flex items-center gap-1.5">
                <span>👤</span> Coordinator Insights
              </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel rounded-2xl border border-slate-800 overflow-hidden shadow-lg">
          <div className="px-4 py-3 border-b border-slate-800 bg-slate-900/50 flex items-center justify-between">
            <h3 className="text-xs font-bold text-white flex items-center gap-2">
              <span>🏛️</span> Institutional Performance Comparison
            </h3>
            <span className="text-[10px] text-slate-400">Calls • Positives • Offers</span>
          </div>
                ...
                {comparisons.map((c) => (
                  <tr key={c.college_id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="py-2.5 px-3 font-semibold text-slate-200">
                      <span className="text-blue-400 font-mono text-[11px] mr-1.5">[{c.college_code}]</span>
                      {c.college_name}
                    </td>
                    <td className="py-2.5 px-3 text-center text-slate-300 font-mono">{c.calls}</td>
                    <td className="py-2.5 px-3 text-center text-emerald-400 font-semibold font-mono">{c.positives}</td>
                    <td className="py-2.5 px-3 text-center text-amber-400 font-mono">{c.drives_completed}</td>
                    <td className="py-2.5 px-3 text-center text-emerald-400 font-bold font-mono">{c.offers}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="glass-panel rounded-2xl border border-slate-800 overflow-hidden shadow-lg">
          <div className="px-4 py-3 border-b border-slate-800 bg-slate-900/50 flex items-center justify-between">
            <h3 className="text-xs font-bold text-white flex items-center gap-2">
              <span>🏢</span> Industry & Corporate Domain Breakdown
            </h3>
            <span className="text-[10px] text-slate-400">Weekly Tracker Pipeline</span>
          </div>

          <div className="p-4 space-y-3">
            {responsiveness?.industry_distribution?.length > 0 ? (
              responsiveness.industry_distribution.map((ind: any, i: number) => (
                <div key={i} className="flex items-center justify-between bg-slate-900/40 p-2.5 rounded-lg border border-slate-800">
                  <span className="text-xs font-semibold text-slate-200">{ind.type}</span>

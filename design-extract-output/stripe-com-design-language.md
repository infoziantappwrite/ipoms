...
> Extracted from `https://stripe.com` on August 23, 2026
> 2826 elements analyzed
...
|------|-----|-----|-----|-------------|
| Primary | `#533afd` | rgb(83, 58, 253) | hsl(248, 98%, 61%) | 1473 |
...
| Hex | HSL | Usage Count |
|-----|-----|-------------|
| `#000000` | hsl(0, 0%, 0%) | 2750 |
| `#50617a` | hsl(216, 21%, 40%) | 332 |
| `#ffffff` | hsl(0, 0%, 100%) | 271 |
...
| Hex | Contexts | Count |
|-----|----------|-------|
| `#000000` | text, border, background | 2750 |
| `#533afd` | text, border, background | 1473 |
| `#50617a` | text, border | 332 |
| `#061b31` | text, border, background | 314 |
| `#ffffff` | background, text, border | 271 |
...
- **sohne-var** — used for all (2826 elements)
...
**52 grid containers** and **368 flex containers** detected.
...
**128 unique SVG icons** detected. Dominant style: **filled**.
...
**Icon colors:** `#031323`, `var(--hds-color-text-solid)`, `currentColor`, `white`, `black`, `var(--hds-color-surface-bg-quiet)`, `rgb(0, 0, 0)`, `var(--customerLogoColor, #000)`, `var(--customerLogoColor, #171d27)`, `var(--customerLogoColor, #ff6201)`

## Image Style Patterns

| Pattern | Count | Key Styles |
|---------|-------|------------|
| thumbnail | 49 | objectFit: fill, borderRadius: 0px, shape: square |
...
**Aspect ratios:** 4.18:1 (36x), 3:4 (9x), 21:9 (8x), 1:1 (6x), 3:2 (2x), 9:16 (2x), 4:3 (2x), 16:9 (2x)
...
- **ease-out** (564 uses) — `cubic-bezier(0.25, 1, 0.5, 1)`, `cubic-bezier(0.165, 0.84, 0.44, 1)`, `cubic-bezier(0.16, 1, 0.3, 1)`
...
Reading order (top→bottom): cta → nav → nav → nav → pricing-table → hero → pricing-table → content → stats → pricing-table → testimonials → nav → pricing-table → nav → gallery → testimonial → nav → content → pricing-table → pricing-table → gallery → cta → footer
...
**Label:** `flat-illustration` (confidence 0.305)
**Counts:** total 70, svg 44, icon 1, screenshot-like 0, photo-like 2

// MIRROR of frontend/src/lib/contactValidation.ts - the server must enforce the same rules as the screen.
// Change both together (same convention as passwordPolicy.ts). Kept under a different name because
// lib/contactValidation.ts is an older, smaller validator used by scripts/autoCleanMetadataContacts.ts.
/**

 ...
 */
export async function renumberCompanyMetadata(): Promise<{ count: number; message: string }> {
  const companies = await CompanyMetadata.find({ is_deleted: false }).sort({ serial_number: 1, _id: 1 });
  ...
  for (let i = 0; i < bulkOps.length; i += BATCH_SIZE) {
    ...
    await CompanyMetadata.bulkWrite(chunk);
  }

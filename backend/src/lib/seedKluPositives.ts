...
export const KLU_POSITIVES_DATA = [
  // 08/03/2026
...
export async function seedKluCallPositives() {
  try {
    // 1. Find or create KLU college
    let klu = await College.findOne({
      $or: [
        { college_code: 'KLU' },
    ...
    let inserted = 0;
    for (const item of KLU_POSITIVES_DATA) {

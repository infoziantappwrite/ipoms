...
export function registerPendingTaskRoutes(app: Express) {
  ...
  app.get('/api/v1/pending-tasks', async (req: Request, res: Response) => {
    try {
      ...
      const tasks = await PendingTask.find(filter)
        .sort(sortOptions)
  ...
  app.get('/api/v1/pending-tasks/kpi', async (req: Request, res: Response) => {
    try {
      ...
      const allTasks = await PendingTask.find(filter).lean();
  ...
  app.post('/api/v1/pending-tasks', async (req: Request, res: Response) => {
    try {
      const {
        college_id,
        coordinator_id,
      ...
      const highestTask = await PendingTask.findOne({
      ...
      const newTask = await PendingTask.create({
        college_id: new Types.ObjectId(String(college_id)),
        coordinator_id: new Types.ObjectId(String(targetCoordinatorId)),
      ...
      const populated = await PendingTask.findById(newTask._id)
  ...
  app.patch('/api/v1/pending-tasks/:id', async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      ...
      const existingTask = await PendingTask.findOne({ _id: id, is_deleted: false });
      if (!existingTask) {
      ...
      if (company_name !== undefined) existingTask.company_name = company_name.trim();
      if (company_id !== undefined) existingTask.company_id = company_id ? new Types.ObjectId(String(company_id)) : (null as any);
      if (jd_received_date !== undefined) existingTask.jd_received_date = jd_received_date ? new Date(jd_received_date) : null;
      if (db_shared_date !== undefined) existingTask.db_shared_date = db_shared_date ? new Date(db_shared_date) : null;
      ...
      if (serial_no !== undefined) existingTask.serial_no = Number(serial_no);

      if (authUserId && Types.ObjectId.isValid(String(authUserId))) {
        existingTask.updated_by = new Types.ObjectId(String(authUserId));
      }

      await existingTask.save();

      const updated = await PendingTask.findById(existingTask._id)
        .populate('college_id', 'college_name college_code logo_url')
        .populate('coordinator_id', 'full_name official_email')
        .populate('company_id', 'company_name domain logo_url');
  ...
  app.delete('/api/v1/pending-tasks/:id', async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      if (!Types.ObjectId.isValid(id)) {
        return res.status(400).json({
  ...
  app.post('/api/v1/pending-tasks/batch-delete', async (req: Request, res: Response) => {
    try {
      const { ids } = req.body;
      ...
      const validIds = ids.filter((id) => Types.ObjectId.isValid(String(id))).map((id) => new Types.ObjectId(String(id)));
  ...
  app.post('/api/v1/pending-tasks/batch-update', async (req: Request, res: Response) => {
    try {
      ...
      const setObj: any = { updated_at: new Date() };
      const authUserId = (req as any).user?.userId || (req as any).user?._id;
      if (authUserId && Types.ObjectId.isValid(String(authUserId))) {
        setObj.updated_by = new Types.ObjectId(String(authUserId));
      }

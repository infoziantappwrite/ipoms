...
async function main() {
  ...
  // Section Groupings
  const pipelineRows: typeof allWeekly = [];
  const inProgressRows: typeof allWeekly = [];
  const upcomingDriveRows: typeof allWeekly = [];
  const driveInProgressRows: typeof allWeekly = [];
  const completedRows: typeof allWeekly = [];
  const otherRows: typeof allWeekly = [];
  ...
  const collegeBreakdown: Record<string, {
    pipeline: number;
    in_progress: number;
    upcoming: number;
  ...
  const activeJdInProgress = activeJdReceived.filter(
    (l) => l.pipeline_section === 'in_progress' || l.pipeline_section === 'in progress'
  );
  const activeJdUpcoming = activeJdReceived.filter(
    (l) =>
      l.pipeline_section === 'upcoming_drive' ||
      l.pipeline_section === 'upcoming_drives' ||
      l.pipeline_section === 'companies_in_drive' ||
      l.pipeline_section === 'in_drive'
  );
  const activeJdDriveInProg = activeJdReceived.filter(
    (l) => l.pipeline_section === 'drive_in_progress' || l.pipeline_section === 'drive in progress'
  );

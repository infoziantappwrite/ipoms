...
async function run() {
  ...
  const dbColleges = await College.find({ is_deleted: { $ne: true } });
  ...
  for (const sheetName of allSheets) {
    ...
    let college = dbColleges.find(
      (c) => c.college_code?.toUpperCase() === upperSheet || c.college_name?.toUpperCase().includes(upperSheet)
    );
    ...
    if (!college) {
      ...
      college = dbColleges.find((c) =>
        c.college_name.toLowerCase().includes(sheetName.toLowerCase()) ||

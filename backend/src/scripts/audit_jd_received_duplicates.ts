...
async function runAudit() {
  ...
  const activeJdLeads = await ActiveLead.find({ lead_type: 'jd_received', is_deleted: false }).lean();
  console.log(`📌 TOTAL ACTIVE LEADS IN 'JD THIS YEAR' TAB: ${activeJdLeads.length}\n`);

  // Cluster by basic normalized key
  const basicClusters = new Map<string, typeof activeJdLeads>();
  for (const lead of activeJdLeads) {
    const key = basicNormalize(lead.company_name);
    if (!basicClusters.has(key)) basicClusters.set(key, []);
    basicClusters.get(key)!.push(lead);
  }

  const basicDuplicates: any[] = [];
  ...
  const canonicalClusters = new Map<string, typeof activeJdLeads>();
  for (const lead of activeJdLeads) {
    const key = cleanCanonicalName(lead.company_name);
    if (!key) continue;
    if (!canonicalClusters.has(key)) canonicalClusters.set(key, []);
  ...
  const weeklyCompanyOccurrences = new Map<string, Array<{ college: string; section: string; rawName: string; role?: string; ctc?: string }>>();
  for (const w of weeklyJd) {
    ...
    if (!weeklyCompanyOccurrences.has(key)) weeklyCompanyOccurrences.set(key, []);
  ...
  const multiCollegeJdCompanies: any[] = [];
  const spellingVariationsInTracker: any[] = [];
  ...
  const allActiveLeads = await ActiveLead.find({ is_deleted: false }).lean();
  const pipelineNames = new Map<string, typeof allActiveLeads[0]>();
  for (const p of allActiveLeads.filter(l => l.lead_type === 'pipeline')) {
    pipelineNames.set(basicNormalize(p.company_name), p);
  }

  const crossTabConflicts: any[] = [];

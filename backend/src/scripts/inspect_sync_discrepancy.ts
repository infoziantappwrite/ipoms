...
async function main() {
  ...
  const weeklySections: Record<string, number> = {};
  ...
  const weeklyJdCompanies = allWeekly.filter((w) =>
  ...
  const activeJdCompanyNames = new Set(
    allActive.filter((a) => a.lead_type === 'jd_received').map((a) => a.company_name.trim().toLowerCase())
  );

  const missingJdInActive = weeklyJdCompanies.filter(
    (w) => !activeJdCompanyNames.has(w.company_name.trim().toLowerCase())
  );

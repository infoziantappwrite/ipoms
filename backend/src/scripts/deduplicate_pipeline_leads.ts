...
function areRolesEquivalent(r1: string, r2: string): boolean {
  ...
  if (!n1 || !n2) return true;
  if (n1 === 'graduate trainee' || n2 === 'graduate trainee') return true;
  if (n1 === 'get' && (n2.includes('get') || n2.includes('graduate engineer trainee'))) return true;
  if (n2 === 'get' && (n1.includes('get') || n1.includes('graduate engineer trainee'))) return true;
  if ((n1 === 'sde' || n1 === 'software engineer' || n1 === 'software developer') &&
      (n2 === 'sde' || n2 === 'software engineer' || n2 === 'software developer')) return true;
...
async function runDeduplication() {
  ...
  for (const lead of allPipeline) {
    ...
    if (!groups.has(coreKey)) groups.set(coreKey, []);
  ...
  const remainingPipeline = await ActiveLead.countDocuments({ lead_type: 'pipeline', is_deleted: false });
  const remainingJd = await ActiveLead.countDocuments({ lead_type: 'jd_received', is_deleted: false });
  const totalActive = await ActiveLead.countDocuments({ is_deleted: false });

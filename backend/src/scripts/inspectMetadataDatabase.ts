...
async function inspectMetadataDatabase() {
  ...
  const count = await CompanyMetadata.countDocuments({ is_deleted: false });
  const binCount = await CompanyMetadata.countDocuments({ is_deleted: true });
  console.log(`📊 Active Companies in 'company_metadata': ${count}`);
  console.log(`🗑️ Recycle Bin Count: ${binCount}\n`);

  const recent = await CompanyMetadata.find({})
    .sort({ created_at: -1 })
    .limit(6);

  recent.forEach((c: any, idx) => {

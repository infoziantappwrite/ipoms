...
function normalizeOutcome(raw: string): { outcome: CallOutcome | null; followUpMonth?: string | null } {
...
async function main() {
  ...
  const college = await College.findOne({ college_code: targetCollegeCode, is_deleted: { $ne: true } });
  if (!college) {
  ...
  await DailyTracker.deleteMany({ college_id: college._id });

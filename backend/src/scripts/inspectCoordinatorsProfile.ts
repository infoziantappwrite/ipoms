...
async function inspectCoordinators() {
  ...
  const summary = coordinators.map((c: any, index: number) => {
    const assignedColleges = (c.assigned_college_ids || []).map((col: any) => col.college_code || col.college_name).join(', ');
    ...
    if (c.full_name) filledFields.push('full_name');
    if (c.username) filledFields.push('username');
    if (c.official_email) filledFields.push('official_email');
    if (hasEmployeeId) filledFields.push('employee_id'); else missingFields.push('employee_id');
    if (hasPrimaryMobile) filledFields.push('primary_mobile'); else missingFields.push('primary_mobile');
    if (hasPersonalEmail) filledFields.push('personal_email'); else missingFields.push('personal_email');
    if (hasSecondaryMobile || hasAlternateMobile) filledFields.push('secondary/alt_mobile'); else missingFields.push('secondary/alt_mobile');
    if (hasAddress) filledFields.push('address'); else missingFields.push('address');
    if (hasCity) filledFields.push('city'); else missingFields.push('city');
    if (hasState) filledFields.push('state'); else missingFields.push('state');
    if (hasPincode) filledFields.push('pincode'); else missingFields.push('pincode');
    if (hasDob) filledFields.push('date_of_birth'); else missingFields.push('date_of_birth');
    if (hasDoj) filledFields.push('date_of_joining'); else missingFields.push('date_of_joining');
    if (hasLinkedIn) filledFields.push('linkedin_profile'); else missingFields.push('linkedin_profile');
    if (hasPhoto) filledFields.push('profile_photo'); else missingFields.push('profile_photo');
    if (hasAssignedColleges) filledFields.push('assigned_colleges'); else missingFields.push('assigned_colleges');
    ...
    if (missingFields.length === 0) fullyCompleteCount++;
    else if (score >= 40) partiallyCompleteCount++;
    else minimalCount++;

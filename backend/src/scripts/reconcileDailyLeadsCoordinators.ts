...
async function reconcileDailyLeads() {
  ...
  const collegeIdMap = new Map<string, any>();
  ...
  const allUsers = await User.find({ account_status: { $nin: ['inactive', 'deactivated', 'blocked'] } });
  const userByEmail = new Map<string, any>();
  for (const u of allUsers) {
    userByEmail.set((u.official_email || '').toLowerCase(), u);
  }

  const collegeCoordinatorMap = new Map<string, any>();
  ...
  const allLeads = await DailyLead.find({ is_deleted: false }).populate('coordinator_id', 'full_name official_email');
  console.log(`🔍 Found ${allLeads.length} total active daily leads in database.`);

  let updatedCount = 0;
  let alreadyCorrectCount = 0;
  const statsByCoord: Record<string, { positives: number; jds: number }> = {};
  ...
  for (const lead of allLeads) {
    ...
    const currentCoordId = lead.coordinator_id && (typeof lead.coordinator_id === 'object' ? String((lead.coordinator_id as any)._id) : String(lead.coordinator_id));
    const targetCoordId = String(assignedCoord._id);

    if (currentCoordId !== targetCoordId) {
      lead.coordinator_id = new Types.ObjectId(targetCoordId);
      await lead.save();
      updatedCount++;
    } else {
      alreadyCorrectCount++;

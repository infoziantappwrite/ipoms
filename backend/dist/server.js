...
const updateNehruWeeklyTracker_1 = require("./scripts/updateNehruWeeklyTracker");
const updateHitsWeeklyTracker_1 = require("./scripts/updateHitsWeeklyTracker");
...
const syncCollegesFromSharepoint_1 = require("./scripts/syncCollegesFromSharepoint");
...
app.get('/health/duplicate-audit', async (req, res) => {
    try {
        const leads = await ActiveLead_1.ActiveLead.find({ is_deleted: false }).lean();
        const weeklyTrackers = await WeeklyTracker_1.WeeklyTracker.find({ is_deleted: false }).select('company_name job_role ctc_lpa pipeline_section academic_year').lean();
        const masterPositives = seedMasterDailyLeads_1.MASTER_POSITIVES_DATA || [];
        const cleanName = (s) => {
            return (s || '')
                .toLowerCase()
                .replace(/[.,/#!$%^&*;:{}=\-_`~()]/g, ' ')
        ...
        const stripCorporateSuffixes = (s) => {
            let n = cleanName(s);
            const suffixes = [
        ...
        const analyzeCompanyList = (items) => {
            ...
            for (const item of items) {
                ...
                if (!strippedGroups.has(key))
                    strippedGroups.set(key, []);
        ...
        const activeLeadsAudit = analyzeCompanyList(leads.map(l => ({ name: l.company_name, role: l.role, ctc: l.ctc, extra: l.academic_year })));
        const masterPositivesAudit = analyzeCompanyList(masterPositives.map(m => ({ name: m.company, role: m.role, ctc: m.ctc, extra: m.collegeCode + ' ' + m.date })));
        const weeklyTrackersAudit = analyzeCompanyList(weeklyTrackers.map(w => ({ name: w.company_name, role: w.job_role, ctc: w.ctc_lpa, extra: w.pipeline_section })));
...
app.all('/api/v1/weekly-tracker/sync-all-past', async (req, res) => {
    try {
        const { syncAllPastDailyToWeekly } = await Promise.resolve().then(() => __importStar(require('./scripts/syncPastDailyToWeekly')));
...
app.get('/api/v1/weekly-tracker/sync-inspection', async (req, res) => {
    try {
        const totalWeekly = await WeeklyTracker_1.WeeklyTracker.countDocuments({ is_deleted: false });
        const pipelineCount = await WeeklyTracker_1.WeeklyTracker.countDocuments({ pipeline_section: 'pipeline', is_deleted: false });
        const statusCounts = await WeeklyTracker_1.WeeklyTracker.aggregate([
            { $match: { is_deleted: false } },
            { $group: { _id: '$current_status_text', count: { $sum: 1 } } },
            { $sort: { count: -1 } },
        ]);
        const dailyPositiveCount = await DailyTracker_1.DailyTracker.countDocuments({ outcome_status: 'invite_mail' });
        const dailyPromotedCount = await DailyTracker_1.DailyTracker.countDocuments({ outcome_status: 'invite_mail', is_promoted_to_weekly: true });
        return res.status(200).json({
            success: true,
            data: {
                totalWeekly,
...
app.all('/api/v1/weekly-tracker/nehru/refresh', async (req, res) => {
    try {
        const result = await (0, updateNehruWeeklyTracker_1.updateNehruWeeklyTracker)();
...
app.all('/api/v1/weekly-tracker/hits/refresh', async (req, res) => {
    try {
        const result = await (0, updateHitsWeeklyTracker_1.updateHitsWeeklyTracker)();
        return res.json(result);
    }
    catch (err) {
        return res.status(500).json({ success: false, error: err.message });
...
app.get('/api/v1/companies/search', async (req, res) => {
    try {
        ...
        if (isRecent) {
            ...
            const highestDoc = await CompanyMetadata_1.CompanyMetadata.findOne({ is_deleted: false }).sort({ serial_number: -1 }).select('serial_number');
...
exports.OFFICIAL_COLLEGE_DEFINITIONS = [
...
async function syncActiveCollegesRoster() {
    try {
        for (const def of exports.OFFICIAL_COLLEGE_DEFINITIONS) {
            let existing = await College_1.College.findOne({
                $or: [
            ...
            if (existing) {
                ...
                if (!existing.logo_url)
                    existing.logo_url = def.logo_url;
                await existing.save();
            }
            else {
                await College_1.College.create({
        ...
        await College_1.College.updateMany({ college_code: { $nin: exports.ACTIVE_COLLEGE_CODES } }, { $set: { status: 'inactive' } });
...
app.get('/api/v1/colleges/focus-matrix', async (req, res) => {
    try {
        ...
        let currentUserId = req.user?.userId || req.user?._id || req.user?.id || null;
        let currentUserEmail = req.user?.email || req.user?.official_email || null;
        if (!currentUserId || !currentUserEmail) {
            ...
            if (authHeader && authHeader.startsWith('Bearer ')) {
                try {
                    ...
                    if (!currentUserEmail)
                        currentUserEmail = decoded.email || decoded.official_email || null;
                }
                catch { }
            }
        }
        ...
        let allColleges = await College_1.College.find({ status: 'active' }).sort({ college_code: 1 });
        if (allColleges.length === 0) {
            await syncActiveCollegesRoster();
            allColleges = await College_1.College.find({ status: 'active' }).sort({ college_code: 1 });
            if (allColleges.length === 0) {
                allColleges = await College_1.College.find({}).sort({ college_code: 1 });
                if (allColleges.length > 0) {
                    await College_1.College.updateMany({}, { $set: { status: 'active' } });
                }
            }
        }
        ...
        if (currentUserId && mongoose_1.Types.ObjectId.isValid(currentUserId)) {
            currentUser = await User_1.User.findById(currentUserId);
        }
        if (!currentUser && currentUserEmail) {
            currentUser = await User_1.User.findOne({ official_email: currentUserEmail.toLowerCase(), is_deleted: false });
            if (currentUser) {
        ...
        for (const coord of activeCoordinators) {
            ...
            if ((normalizedCurrentId && coordIdStr === normalizedCurrentId) ||
        ...
        let myAssignedCollegeIds = (currentUser?.assigned_college_ids || []).map((id) => String(id));
        ...
        const collegesWithOccupancy = allColleges.map((c) => {
            ...
            const otherHandlers = rawOtherHandlers.filter((h) => (!normalizedCurrentId || h.user_id !== normalizedCurrentId) &&
...
app.get('/api/v1/colleges/:id', async (req, res) => {
    try {
        const { id } = req.params;
        let college = null;
        if (mongoose_1.Types.ObjectId.isValid(id)) {
            college = await College_1.College.findById(id);
        }
        else {
...
app.patch('/api/v1/colleges/:id', async (req, res) => {
    try {
        const { id } = req.params;
        if (!mongoose_1.Types.ObjectId.isValid(id)) {
            return res.status(400).json({
        ...
        const college = await College_1.College.findByIdAndUpdate(id, { $set: updatePayload }, { new: true, runValidators: true });
        if (!college) {
            return res.status(404).json({
                success: false,
                error: { code: 'NOT_FOUND', message: 'College not found' },
...
app.post('/api/v1/colleges/sync-sharepoint', async (_req, res) => {
    try {
        const syncResult = await (0, syncCollegesFromSharepoint_1.syncCollegesFromExcel)();
        ...
        try {
            const localFile = path_1.default.resolve(__dirname, '../../Colleges_and_Coordinators.xlsx');
            if (fs_1.default.existsSync(localFile)) {
                const fallbackResult = await (0, syncCollegesFromSharepoint_1.syncCollegesFromExcel)(localFile);
...
app.post('/api/v1/colleges/lock-focus', async (req, res) => {
    try {
        ...
        if (!currentUserId && currentUserEmail) {
            const foundUser = await User_1.User.findOne({ official_email: currentUserEmail.toLowerCase(), is_deleted: false });
            if (foundUser)
                currentUserId = String(foundUser._id);
        }
        if (!currentUserId) {
        ...
        let allActiveColleges = await College_1.College.find({ status: 'active' });
        if (allActiveColleges.length === 0) {
            await syncActiveCollegesRoster();
            allActiveColleges = await College_1.College.find({ status: 'active' });
        }
        const resolvedCollegeDocs = [];
        ...
        for (const rawItem of college_ids) {
            ...
            if (mongoose_1.Types.ObjectId.isValid(itemStr)) {
                ...
                if (!foundDoc) {
                    foundDoc = await College_1.College.findById(itemStr);
                }
            }
            ...
            if (!foundDoc) {
                ...
                if (!foundDoc) {
                    foundDoc = await College_1.College.findOne({ college_code: cleanedCode });
                }
            }
            // 3. Try match by college_name
            if (!foundDoc) {
                const nameClean = itemStr.toLowerCase();
                foundDoc = allActiveColleges.find((c) => c.college_name?.toLowerCase() === nameClean ||
                    c.college_name?.toLowerCase().includes(nameClean) ||
                    nameClean.includes(c.college_name?.toLowerCase()));
                if (!foundDoc) {
                    foundDoc = await College_1.College.findOne({
        ...
        for (const reqId of sanitizedIds) {
            const otherHandlersForCollege = otherCoordinators.filter((oc) => oc.assigned_college_ids.some((c) => String(c?._id || c) === String(reqId)));
        ...
        await College_1.College.updateMany({ assigned_coordinator_ids: user._id, _id: { $nin: resolvedObjectIds } }, { $pull: { assigned_coordinator_ids: user._id } });
        // Add coordinator to newly selected colleges
        await College_1.College.updateMany({ _id: { $in: resolvedObjectIds } }, { $addToSet: { assigned_coordinator_ids: user._id } });
...
app.get('/api/v1/coordinators', async (req, res) => {
    try {
        const allColleges = await College_1.College.find({ status: 'active', is_deleted: false }).select('_id college_name college_code location logo_url');
        ...
        const formatted = coordinators.map((coord) => {
            ...
            if (!defaultCodes) {
                for (const [key, val] of Object.entries(DEFAULT_COORDINATOR_COLLEGE_MAP)) {
                    ...
                    if ((email && email.includes(keyPrefix)) ||
            ...
            const matchedColleges = defaultCodes && defaultCodes.length > 0
                ? allColleges.filter((col) => defaultCodes.some((cd) => cd.toUpperCase() === col.college_code?.toUpperCase() ||
...
app.get('/api/v1/daily-tracker/today', async (req, res) => {
    try {
        const rawCollegeId = req.query.college_id;
        const rawCoordId = req.query.coordinator_id;
        const isSuper = (0, routePolicy_1.isSupervisor)(req);
        const selfId = req.user?.userId;
        const filter = {
            session_date: buildSessionDate(),
            is_finalized: false,
        };
        if (rawCollegeId && rawCollegeId !== 'all' && mongoose_1.Types.ObjectId.isValid(rawCollegeId)) {
            filter.college_id = new mongoose_1.Types.ObjectId(String(rawCollegeId));
        }
        if (rawCoordId && rawCoordId !== 'all' && mongoose_1.Types.ObjectId.isValid(rawCoordId)) {
            filter.coordinator_id = new mongoose_1.Types.ObjectId(String(rawCoordId));
        }
        else if (!rawCoordId && !isSuper) {
            if (selfId && mongoose_1.Types.ObjectId.isValid(selfId)) {
                filter.coordinator_id = new mongoose_1.Types.ObjectId(String(selfId));
            }
        }
        ...
        const enriched = uniqueRows.map((row, idx) => {
            ...
            const coord = obj.coordinator_id;
            const coll = obj.college_id;
...
app.post('/api/v1/daily-tracker/check-metadata-batch', async (req, res) => {
    try {
        ...
        const trimmedNames = company_names.map((n) => String(n || '').trim()).filter(Boolean);
        if (trimmedNames.length === 0) {
            return res.json({ success: true, data: { existing_names: [], new_names: [] } });
        }
        // Match exact or case-insensitive
        const regexes = trimmedNames.map((name) => new RegExp(`^${escapeRegex(name)}$`, 'i'));
        const matchedDocs = await CompanyMetadata_1.CompanyMetadata.find({
...
app.post('/api/v1/daily-tracker/bulk-paste', async (req, res) => {
    try {
        ...
        const existingTrackerRows = await DailyTracker_1.DailyTracker.find({
            coordinator_id: new mongoose_1.Types.ObjectId(coordinator_id),
            college_id: new mongoose_1.Types.ObjectId(college_id),
            session_date: today,
            is_finalized: false,
        }).select('company_name mobile_number email_id');
        const existingKeys = new Set(existingTrackerRows.map((r) => {
        ...
        const newMetadataCompanies = [];
        const newMetadataIds = [];
        ...
        for (const item of incomingRows) {
            ...
            const mobList = mobileNumber ? mobileNumber.split(/[,;/]+/).map((s) => s.trim()).filter(Boolean) : [];
            const emailList = emailId ? emailId.split(/[,;/]+/).map((s) => s.trim().toLowerCase()).filter(Boolean) : [];
            if (!company) {
...
app.post('/api/v1/daily-tracker/copy-from-history', async (req, res) => {
    try {
        ...
        if (Array.isArray(inputRows) && inputRows.length > 0) {
            ...
            sourceRows = await DailyTracker_1.DailyTracker.find({
                _id: { $in: row_ids.map((id) => new mongoose_1.Types.ObjectId(id)) },
            }).lean();
        }
        ...
        const existingKeys = new Set(existingRows.map((r) => `${(r.company_name || '').trim().toLowerCase()}_${(r.mobile_number || '').trim()}`));
        const toInsert = [];
        ...
        for (const src of sourceRows) {
            ...
            let companyId = src.company_id && mongoose_1.Types.ObjectId.isValid(src.company_id) ? new mongoose_1.Types.ObjectId(src.company_id) : undefined;
            if (!companyId) {
                let comp = await CompanyMetadata_1.CompanyMetadata.findOne({
                ...
                if (!comp) {
                    const highestDoc = await CompanyMetadata_1.CompanyMetadata.findOne({ serial_number: { $gt: 0 } })
                        ...
                        .select('serial_number');
                    const nextSerial = (highestDoc?.serial_number || 0) + 1;
                    const mobList = mob ? mob.split(/[,;/]+/).map((s) => s.trim()).filter(Boolean) : [];
                    const emailList = src.email_id ? src.email_id.split(/[,;/]+/).map((s) => s.trim().toLowerCase()).filter(Boolean) : [];
                    comp = await CompanyMetadata_1.CompanyMetadata.create({
                        serial_number: nextSerial,
...
app.patch('/api/v1/daily-tracker/:id', async (req, res) => {
    try {
        ...
        if (row.company_id || row.company_name) {
            (async () => {
                try {
                    let meta = row.company_id ? await CompanyMetadata_1.CompanyMetadata.findById(row.company_id) : null;
                    if (!meta && row.company_name) {
                        meta = await CompanyMetadata_1.CompanyMetadata.findOne({
        ...
        (async () => {
            try {
                ...
                if (outcome_status !== undefined && !isPositiveOutcome) {
                    await DailyLead_1.DailyLead.updateMany({
                        $or: [
                            { daily_tracker_id: row._id },
                            {
                                college_id: row.college_id,
                    ...
                    await WeeklyTracker_1.WeeklyTracker.updateMany({
                        $or: [
                            { daily_tracker_id: row._id },
                            {
                                college_id: row.college_id,
                ...
                if (isPositiveOutcome) {
                    ...
                    const existingLead = await DailyLead_1.DailyLead.findOne({
                        $or: [
                            { daily_tracker_id: row._id },
                            {
                                college_id: row.college_id,
                    ...
                    if (existingLead) {
                        ...
                        if (row.comments)
                            existingLead.remarks = row.comments;
                ...
                if (company_name !== undefined || hr_name !== undefined || mobile_number !== undefined || email_id !== undefined || comments !== undefined) {
                    ...
                    const wtRows = await WeeklyTracker_1.WeeklyTracker.find({
                        $or: [
                            { daily_tracker_id: row._id },
                            {
                                college_id: row.college_id,
...
app.delete('/api/v1/daily-tracker/bulk', async (req, res) => {
    try {
        ...
        if (college_id && college_id !== 'all') {
            filter.college_id = new mongoose_1.Types.ObjectId(college_id);
        }
        // Determine scope
        if (scope === 'today') {
            ...
            if (coordinator_id) {
                filter.coordinator_id = new mongoose_1.Types.ObjectId(coordinator_id);
            }
        }
        ...
        const rowsToDelete = await DailyTracker_1.DailyTracker.find(filter);
        ...
        const result = await DailyTracker_1.DailyTracker.deleteMany(filter);
        return res.status(200).json({
            success: true,
...
app.all('/api/v1/daily-tracker/sync-coordinators', async (req, res) => {
    try {
        const currentWeekMonday = getWeekMondayKey();
        const mohanaDoc = await User_1.User.findOne({
            $or: [
        ...
        if (mohanaDoc) {
            ...
            const targetCollegeDocs = await College_1.College.find({
            ...
            if (targetCollegeIds.length > 0) {
                ...
                if (acetDoc) {
                    const r1 = await DailyTracker_1.DailyTracker.updateMany({ college_id: acetDoc._id }, { $set: { coordinator_id: mohanaDoc._id } });
                    updatedCount += r1.modifiedCount;
                }
                const adminUsersList = await User_1.User.find({
                    role_codes: { $in: ['ADMINISTRATOR', 'ADMIN'] },
                }).select('_id');
                const adminIds = adminUsersList.map((u) => u._id);
                const r2 = await DailyTracker_1.DailyTracker.updateMany({
...
app.get('/api/v1/daily-tracker/history', async (req, res) => {
    try {
        ...
        if (coordinator_id && coordinator_id !== 'all' && mongoose_1.Types.ObjectId.isValid(coordinator_id)) {
            filter.coordinator_id = new mongoose_1.Types.ObjectId(String(coordinator_id));
        }
        if (rawCollegeId && rawCollegeId !== 'all' && mongoose_1.Types.ObjectId.isValid(String(rawCollegeId))) {
            filter.college_id = new mongoose_1.Types.ObjectId(String(rawCollegeId));
        }
        const rows = await DailyTracker_1.DailyTracker.find(filter)
            .populate('coordinator_id', 'full_name official_email')
...
const getActiveDaysHandler = async (req, res) => {
    try {
        const rawCoordinatorId = req.query.coordinator_id;
        const rawCollegeId = req.query.college_id;
        ...
        if (rawCoordinatorId && rawCoordinatorId !== 'all' && mongoose_1.Types.ObjectId.isValid(rawCoordinatorId)) {
            filter.coordinator_id = new mongoose_1.Types.ObjectId(String(rawCoordinatorId));
        }
        if (rawCollegeId && rawCollegeId !== 'all' && mongoose_1.Types.ObjectId.isValid(rawCollegeId)) {
            filter.college_id = new mongoose_1.Types.ObjectId(String(rawCollegeId));
        }
        const records = await DailyTracker_1.DailyTracker.find(filter).select('session_date day created_at year month').lean();
        ...
        if (daysSet.size === 0 && (filter.coordinator_id || filter.college_id)) {
            const fallbackRecords = await DailyTracker_1.DailyTracker.find({
                $or: [
                    { session_date: { $gte: startOfMonth, $lte: endOfMonth } },
                    { created_at: { $gte: startOfMonth, $lte: endOfMonth } },
                    { year, month },
...
app.get('/api/v1/daily-tracker/kpi', async (req, res) => {
    try {
        ...
        if (rawCollegeId && rawCollegeId !== 'all' && mongoose_1.Types.ObjectId.isValid(rawCollegeId)) {
            baseFilter.college_id = new mongoose_1.Types.ObjectId(String(rawCollegeId));
        }
        if (rawCoordId && rawCoordId !== 'all' && mongoose_1.Types.ObjectId.isValid(rawCoordId)) {
            baseFilter.coordinator_id = new mongoose_1.Types.ObjectId(String(rawCoordId));
        }
        else if (!rawCoordId && !isSuper) {
            const selfId = (0, routePolicy_1.scopeToSelf)(req, rawCoordId);
            if (selfId && mongoose_1.Types.ObjectId.isValid(selfId)) {
                baseFilter.coordinator_id = new mongoose_1.Types.ObjectId(String(selfId));
            }
        }
        const [total, completed, noResponse, followUp, positive] = await Promise.all([
            ...
            DailyTracker_1.DailyTracker.countDocuments({ ...baseFilter, outcome_status: 'invite_mail', is_skipped: false }),
        ]);
        return res.status(200).json({
            success: true,
            data: {
...
const handlePendingFollowups = async (req, res) => {
    try {
        ...
        if (targetCollegeIds.length === 0 && userId) {
            const userDoc = await User_1.User.findById(userId).select('assigned_college_ids role_codes').lean();
            if (userDoc && Array.isArray(userDoc.assigned_college_ids) && userDoc.assigned_college_ids.length > 0) {
                ...
                targetCollegeIds = userDoc.assigned_college_ids.map((id) => String(id));
            }
        }
        const wtFilter = {
            is_deleted: { $ne: true },
        ...
        if (targetCollegeIds.length > 0) {
            wtFilter.college_id = { $in: targetCollegeIds.map((id) => new mongoose_1.Types.ObjectId(id)) };
        }
        if (requestedCoordId && requestedCoordId !== 'all') {
            wtFilter.coordinator_id = new mongoose_1.Types.ObjectId(requestedCoordId);
        }
        const rows = await WeeklyTracker_1.WeeklyTracker.find(wtFilter)
        ...
        for (const item of categorized) {
            ...
            c.total_count += 1;
            if (item.urgency === 'due_now')
                c.due_now_count += 1;
            if (item.urgency === 'overdue')
                c.overdue_count += 1;
        }
        if (targetCollegeIds.length > 0) {
            const allTargetColleges = await College_1.College.find({
                _id: { $in: targetCollegeIds.map((id) => new mongoose_1.Types.ObjectId(id)) },
                is_deleted: { $ne: true },
...
app.get('/api/v1/weekly-tracker', async (req, res) => {
    try {
        ...
        if (dupIdsToClean.length > 0) {
            WeeklyTracker_1.WeeklyTracker.deleteMany({ _id: { $in: dupIdsToClean } }).catch((e) => console.error('[WT-1] Failed to clean duplicate rows:', e));
        }
...
app.post('/api/v1/weekly-tracker', async (req, res) => {
    try {
        ...
        if (resolvedCompanyId && mongoose_1.Types.ObjectId.isValid(String(resolvedCompanyId))) {
            const existingMeta = await CompanyMetadata_1.CompanyMetadata.findById(resolvedCompanyId);
    ...
    try {
        ...
        (async () => {
            try {
                const escapedName = escapeRegex(row.company_name.trim());
                const leadUpdate = {};
                if (patchData.company_name !== undefined)
                    leadUpdate.company_name = row.company_name.trim();
                if (patchData.job_role !== undefined)
                    leadUpdate.job_role = row.job_role?.trim() || '';
                if (patchData.ctc_lpa !== undefined)
                    leadUpdate.ctc = row.ctc_lpa?.trim() || '';
                if (patchData.eligible_batch !== undefined)
                    leadUpdate.eligible_batch = row.eligible_batch?.trim() || '';
                if (patchData.current_status_text !== undefined || patchData.remarks !== undefined) {
                    leadUpdate.remarks = row.current_status_text?.trim() || row.remarks?.trim() || '';
                }
                ...
                if (row.daily_tracker_id) {
                    const dtUpdate = {};
                    if (patchData.company_name !== undefined)
                        dtUpdate.company_name = row.company_name.trim();
                    ...
                    if (Object.keys(dtUpdate).length > 0) {
                        await DailyTracker_1.DailyTracker.updateOne({ _id: row.daily_tracker_id, is_deleted: false }, { $set: dtUpdate });
                    }
                }
            }
            catch (cascadeErr) {
    ...
    try {
        ...
        (async () => {
            try {
                ...
                if (row.daily_tracker_id) {
                    await DailyTracker_1.DailyTracker.updateOne({ _id: row.daily_tracker_id, is_deleted: false }, { $set: { is_deleted: true, deleted_at: new Date() } });
                }
            }
            catch (cascadeErr) {
...
app.post('/api/v1/weekly-tracker/batch-delete', async (req, res) => {
    try {
        ...
        const objectIds = ids
            .filter((id) => mongoose_1.Types.ObjectId.isValid(id))
            .map((id) => new mongoose_1.Types.ObjectId(id));
        const rowsToDelete = await WeeklyTracker_1.WeeklyTracker.find({ _id: { $in: objectIds } });
            ...
            try {
                for (const r of rowsToDelete) {
                    ...
                    if (r.daily_tracker_id) {
                        await DailyTracker_1.DailyTracker.updateOne({ _id: r.daily_tracker_id, is_deleted: false }, { $set: { is_deleted: true, deleted_at: new Date() } });
                    }
                }
            }
            catch (cascadeErr) {
...
app.get('/api/v1/weekly-tracker/kpi', async (req, res) => {
    try {
        ...
        const queryCollegeIds = [];
        if (college_id === 'all') {
            const allCols = await College_1.College.find({ is_active: { $ne: false } });
            allCols.forEach((ac) => queryCollegeIds.push(ac._id));
        }
        else if (mongoose_1.Types.ObjectId.isValid(String(college_id))) {
            queryCollegeIds.push(new mongoose_1.Types.ObjectId(String(college_id)));
            const targetCol = await College_1.College.findById(college_id);
            if (targetCol) {
                const sameCodeCols = await College_1.College.find({
        ...
        if (academic_year && academic_year !== 'all') {
            baseFilter.academic_year = Number(academic_year) || (await (0, academicYear_1.getCurrentAcademicYear)());
        }
        const todayEnd = new Date();
        todayEnd.setHours(23, 59, 59, 999);
        const [completedRows, driveInProgressCount, upcomingDrivesCount, inProgressCount, pipelineCount, topCount, rejectedCount, followUpsDueCount,] = await Promise.all([
...
app.post('/api/v1/weekly-tracker/sync-daily-positives', async (req, res) => {
    try {
        ...
        const existingAll = await WeeklyTracker_1.WeeklyTracker.find({
            college_id: { $in: queryCollegeIds },
        ...
        if (dupIdsToDelete.length > 0) {
            await WeeklyTracker_1.WeeklyTracker.deleteMany({ _id: { $in: dupIdsToDelete } });
        }
        // Daily Leads filter: Positive section leads for these college(s)
        const dailyLeadFilter = {
            college_id: { $in: queryCollegeIds },
            lead_type: 'positive',
            is_deleted: false,
        };
        if (coordinator_id && mongoose_1.Types.ObjectId.isValid(String(coordinator_id))) {
            dailyLeadFilter.coordinator_id = new mongoose_1.Types.ObjectId(String(coordinator_id));
        }
        const positiveDailyLeads = await DailyLead_1.DailyLead.find(dailyLeadFilter).sort({ lead_date: 1, created_at: 1 });
        ...
        const maxPipelineRow = await WeeklyTracker_1.WeeklyTracker.findOne({
            college_id: { $in: queryCollegeIds },
            academic_year: targetYear,
            pipeline_section: 'pipeline',
            is_deleted: false,
        ...
        let nextOrderIndex = maxPipelineRow && typeof maxPipelineRow.order_index === 'number'
            ...
            : (await WeeklyTracker_1.WeeklyTracker.countDocuments({
                college_id: { $in: queryCollegeIds },
                academic_year: targetYear,
                pipeline_section: 'pipeline',
                is_deleted: false,
        ...
        let syncedCount = 0;
        const inBatchSyncedNames = new Set(seenNames);
        for (const lead of positiveDailyLeads) {
            ...
            if (inBatchSyncedNames.has(normalizedKey)) {
                ...
                if (lead.job_role || lead.ctc) {
                    ...
                    const existingRow = await WeeklyTracker_1.WeeklyTracker.findOne({
                        college_id: { $in: queryCollegeIds },
                        academic_year: targetYear,
            ...
            if (!existing) {
                ...
                let metaContacts = {};
                if (lead.company_id && mongoose_1.Types.ObjectId.isValid(String(lead.company_id))) {
                    const meta = await CompanyMetadata_1.CompanyMetadata.findById(lead.company_id).lean();
                    if (meta) {
                        metaContacts = {
                ...
                let dtContact = null;
                if (lead.daily_tracker_id && mongoose_1.Types.ObjectId.isValid(String(lead.daily_tracker_id))) {
                    dtContact = await DailyTracker_1.DailyTracker.findById(lead.daily_tracker_id).lean();
                }
                ...
                await WeeklyTracker_1.WeeklyTracker.create({
                    ...
                    coordinator_id: lead.coordinator_id || (coordinator_id ? new mongoose_1.Types.ObjectId(String(coordinator_id)) : new mongoose_1.Types.ObjectId('6a847199fa3bf51271bc14eb')),
                    company_id: lead.company_id || new mongoose_1.Types.ObjectId(),
...
app.get('/api/v1/daily-leads', async (req, res) => {
    try {
        ...
        const leadsRaw = await DailyLead_1.DailyLead.find(filter)
        ...
        const usersList = await User_1.User.find({ account_status: { $nin: ['inactive', 'deactivated', 'blocked'] } }).lean();
        const collegesList = await College_1.College.find({}).lean();
...
app.post('/api/v1/daily-leads', async (req, res) => {
    try {
        ...
        let resolvedCoordinatorId = (req.body.coordinator_id && mongoose_1.Types.ObjectId.isValid(String(req.body.coordinator_id)) ? String(req.body.coordinator_id) : undefined) ||
            req.user?.userId;
...
function formatLeadUpperTime(timeInput) {
    if (!timeInput)
        return '';
...
app.patch('/api/v1/daily-leads/:id', async (req, res) => {
    try {
        ...
        if ((0, routePolicy_1.refuseForeignOwner)(req, res, String(lead.coordinator_id), 'You can only edit your own leads.'))
            return;
        const allowedFields = [
            'company_name',
            'job_role',
        ...
        (async () => {
            try {
                ...
                const wtUpdate = {};
                if (patchData.company_name !== undefined)
                    wtUpdate.company_name = lead.company_name.trim();
                if (patchData.job_role !== undefined)
                    wtUpdate.job_role = lead.job_role?.trim() || '';
                if (patchData.ctc !== undefined)
                    wtUpdate.ctc_lpa = lead.ctc?.trim() || '';
                if (patchData.eligible_batch !== undefined)
                    wtUpdate.eligible_batch = lead.eligible_batch?.trim() || '';
                if (patchData.remarks !== undefined)
                    wtUpdate.current_status_text = lead.remarks?.trim() || '';
                ...
                if (lead.daily_tracker_id && (patchData.company_name !== undefined || patchData.remarks !== undefined)) {
                    await DailyTracker_1.DailyTracker.updateOne({ _id: lead.daily_tracker_id, is_deleted: false }, {
                        $set: {
...
app.post('/api/v1/daily-leads/:id/move-to-jd', async (req, res) => {
    try {
        ...
        if ((0, routePolicy_1.refuseForeignOwner)(req, res, String(lead.coordinator_id), 'You can only move your own leads to JD Received.'))
            return;
        ...
        if (leadDateStr === targetDateStr) {
            ...
            lead.is_moved_to_jd = true;
            await lead.save();
            const existingJd = await DailyLead_1.DailyLead.findOne({
            ...
            if (!jdLead) {
                jdLead = await DailyLead_1.DailyLead.create({
                    lead_type: 'jd_received',
                    college_id: lead.college_id,
            ...
            const populated = await DailyLead_1.DailyLead.findById(jdLead._id)
                .populate('college_id', 'college_name college_code')
                .populate('coordinator_id', 'full_name official_email');
            return res.status(200).json({
                success: true,
...
app.delete('/api/v1/daily-leads/:id', async (req, res) => {
    try {
        ...
        if ((0, routePolicy_1.refuseForeignOwner)(req, res, String(lead.coordinator_id), 'You can only delete your own leads.'))
            return;
        ...
        (async () => {
            try {
                ...
                if (lead.daily_tracker_id) {
                    await DailyTracker_1.DailyTracker.updateOne({ _id: lead.daily_tracker_id, is_deleted: false }, { $set: { is_deleted: true, deleted_at: new Date() } });
                }
            }
            catch (cascadeErr) {
...
app.post('/api/v1/daily-leads/batch-delete', async (req, res) => {
    try {
        const { ids } = req.body;
        if (!Array.isArray(ids) || ids.length === 0) {
            return res.status(400).json({
        ...
        const objectIds = ids
            .filter((id) => mongoose_1.Types.ObjectId.isValid(id))
            .map((id) => new mongoose_1.Types.ObjectId(id));
        const leadsToDelete = await DailyLead_1.DailyLead.find({ _id: { $in: objectIds } });
        await DailyLead_1.DailyLead.updateMany({ _id: { $in: objectIds } }, { $set: { is_deleted: true, deleted_at: new Date() } });
    ...
    try {
        ...
        for (const call of positiveCalls) {
            ...
            if (leadType === 'positive') {
                const alreadyMovedToJd = await DailyLead_1.DailyLead.findOne({
    ...
    try {
        ...
        if (company_name && Array.isArray(college_ids) && college_ids.length > 0) {
            ...
            for (const targetCollegeId of validCollegeIds) {
                ...
                let existing = await DailyLead_1.DailyLead.findOne({
                    lead_type: 'jd_received',
...
function extractCtcNumbersHelper(ctcStr) {
    if (!ctcStr)
        return [];
...
function matchesMinCtcHelper(ctcStr, minCtc, includeCompetitive = false) {
    if (minCtc === null || minCtc === undefined || minCtc <= 0)
        return true;
...
app.post('/api/v1/reports/generate', async (req, res) => {
    try {
        ...
        const isConsolidatedAllowed = template_type === 'active_leads' ||
        ...
        if (template_type === 'active_leads') {
            ...
            let wtRows = await WeeklyTracker_1.WeeklyTracker.find(wtFilter).sort({ createdAt: -1 }).lean();
            if (wtRows.length === 0 && college_id && college_id !== 'all') {
                delete wtFilter.college_id;
                wtFilter.college_id = { $in: activeCollegeObjectIds };
                wtRows = await WeeklyTracker_1.WeeklyTracker.find(wtFilter).sort({ createdAt: -1 }).lean();
            }
            ...
            const dlRows = await DailyLead_1.DailyLead.find(dlFilter).sort({ createdAt: -1 }).lean();
            ...
            for (const r of wtRows) {
                ...
                const roleStr = (r.job_role || r.role || '').trim();
                const ctcStr = (r.ctc_lpa || r.salary_package || r.ctc || '').trim();
            ...
            if (companyMap.size === 0) {
                ...
                const fallbackLeads = await ActiveLead_1.ActiveLead.find(alFilter).sort({ company_name: 1 }).lean();
            ...
            let activeLeadsList = allCategorizedCompanies.filter((c) => {
                if (c.source === 'jd_received' && includeJd)
                    return true;
                if (c.source === 'pipeline' && includePipeline)
                    return true;
                return false;
            });
        ...
        if (template_type === 'daily_positives') {
            ...
            if (Array.isArray(req.body.custom_daily_leads) && req.body.custom_daily_leads.length > 0) {
                ...
                let rawLeads = await DailyLead_1.DailyLead.find(query)
                    .sort({ lead_date: -1, createdAt: -1 })
                    .populate('coordinator_id', 'full_name official_email username');
                if (rawLeads.length === 0 && academic_year && academic_year !== 'all') {
                    const fallbackQuery = { ...query };
                    delete fallbackQuery.eligible_batch;
                    rawLeads = await DailyLead_1.DailyLead.find(fallbackQuery)
                        .sort({ lead_date: -1, createdAt: -1 })
                        .populate('coordinator_id', 'full_name official_email username');
                }
                if (rawLeads.length === 0 && targetDateStr === 'all') {
                    rawLeads = await DailyLead_1.DailyLead.find({ lead_type: 'positive', is_deleted: false })
                        .sort({ lead_date: -1, createdAt: -1 })
                        .populate('coordinator_id', 'full_name official_email username');
                }
                leadsData = rawLeads.map((r, idx) => ({
                    s_no: idx + 1,
                    _id: String(r._id),
        ...
        if (template_type === 'daily_jd_received') {
            ...
            if (Array.isArray(req.body.custom_daily_leads) && req.body.custom_daily_leads.length > 0) {
                ...
                if (rawLeads.length === 0 && targetDateStr === 'all') {
                    rawLeads = await DailyLead_1.DailyLead.find({ lead_type: 'jd_received', is_deleted: false })
                        .sort({ lead_date: -1, createdAt: -1 })
                        .populate('coordinator_id', 'full_name official_email username');
                }
                leadsData = rawLeads.map((r, idx) => ({
        ...
        if (isMultiCollegeWeekly) {
            let targetCollegesList = [];
            if (Array.isArray(req.body.college_ids) && req.body.college_ids.length > 0) {
                const queryIds = [];
                const codeQueries = [];
                req.body.college_ids.forEach((id) => {
                    if (mongoose_1.Types.ObjectId.isValid(String(id))) {
                        queryIds.push(new mongoose_1.Types.ObjectId(String(id)));
                    }
                    else if (typeof id === 'string') {
                        codeQueries.push(id.toUpperCase());
                    }
                });
                targetCollegesList = await College_1.College.find({
                    $or: [
                        { _id: { $in: queryIds } },
            ...
            const colleges_data = await Promise.all(targetCollegesList.map(async (col) => {
                const cFilter = {
                    college_id: col._id,
                    is_deleted: { $ne: true },
                };
        ...
        let [completedRows, driveInProgressRows, inDriveRows, inProgressRows, pipelineRows, topCompaniesRows, rejectedCompaniesRows, onHoldByCollegeRows, onHoldByHrRows, totalCalls, positiveCalls, notHiringCalls, totalJds,] = await Promise.all([
            ...
            DailyTracker_1.DailyTracker.countDocuments({ ...dtFilter, outcome_status: { $in: ['not_hiring', 'hiring_freezed'] } }),
        ...
        if (custom_weekly_companies && typeof custom_weekly_companies === 'object') {
            ...
            if (Array.isArray(custom_weekly_companies.pipeline))
                pipelineRows = custom_weekly_companies.pipeline;
            if (Array.isArray(custom_weekly_companies.top_companies))
                topCompaniesRows = custom_weekly_companies.top_companies;
            if (Array.isArray(custom_weekly_companies.rejected_companies))
                rejectedCompaniesRows = custom_weekly_companies.rejected_companies;
            if (Array.isArray(custom_weekly_companies.on_hold_by_college))
                onHoldByCollegeRows = custom_weekly_companies.on_hold_by_college;
            if (Array.isArray(custom_weekly_companies.on_hold_by_hr))
                onHoldByHrRows = custom_weekly_companies.on_hold_by_hr;
        }
            ...
            const filterItem = (r) => {
                if (minCtcVal && !matchesMinCtcHelper(r.ctc_lpa || r.ctc, minCtcVal, inclComp))
                    return false;
                if (nameQ && !(r.company_name || '').toLowerCase().includes(nameQ))
                    return false;
                if (typeQ && typeQ !== 'all' && !(r.company_type || '').toLowerCase().includes(typeQ))
                    return false;
                if (statusQ && statusQ !== 'all') {
                    const text = ((r.current_status_text || r.status || '') + ' ' + (r.remarks || '')).toLowerCase();
                    if (!text.includes(statusQ))
                        return false;
                }
                return true;
            };
...
app.get('/api/v1/dashboard/coordinator', async (req, res) => {
    try {
        ...
        for (const row of todayTrackerRows) {
            ...
            const isPositive = row.outcome_status && DailyTracker_1.POSITIVE_OUTCOMES.includes(row.outcome_status);
            if (isPositive) {
                todayPositives++;
            }
            const b = OUTCOME_BUCKET[String(row.outcome_status || '')];
            if (b === 'positive')
                todayOutcomes.positive++;
            else if (b === 'not_hiring')
                todayOutcomes.not_hiring++;
            else if (b === 'negative')
                todayOutcomes.negative++;
            else if (b === 'follow_up')
                todayOutcomes.follow_up++;
...
app.get('/api/v1/dashboard/coordinator/clock-duration', async (req, res) => {
    try {
        const coordinatorId = (0, routePolicy_1.scopeToSelf)(req, req.query.coordinator_id);
        ...
        for (const row of trackerRows) {
            ...
            const b = OUTCOME_BUCKET[String(row.outcome_status || '')];
            if (b === 'positive')
                dayOutcomes.positive++;
            else if (b === 'not_hiring')
                dayOutcomes.not_hiring++;
            else if (b === 'negative')
                dayOutcomes.negative++;
            else if (b === 'follow_up')
                dayOutcomes.follow_up++;
            const stamp = row.call_start_time || row.created_at;
            if (stamp) {
                const t = new Date(stamp).getTime();
                if (!isNaN(t)) {
            ...
            if (row.college_id) {
                const cId = row.college_id._id?.toString() || row.college_id.toString();
                const cName = row.college_id.college_name || 'Partner College';
                const cCode = row.college_id.college_code || '';
...
app.get('/api/v1/dashboard/coordinator/duration-history', async (req, res) => {
    try {
        const coordinatorId = (0, routePolicy_1.scopeToSelf)(req, req.query.coordinator_id);
        const view = req.query.view || 'monthly';
        const reqYear = parseInt(req.query.year, 10) || 2026;
        const reqMonth = parseInt(req.query.month, 10) || 9; // default September 2026
        let coordinator = null;
        if (coordinatorId) {
            coordinator = await User_1.User.findById(coordinatorId);
        }
        ...
        if (view === 'monthly') {
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                const dayRows = trackerRows.filter((r) => {
                    if (r.day === d && (!r.month || r.month === reqMonth) && (!r.year || r.year === reqYear))
                        return true;
                    if (r.session_date) {
                        const dt = new Date(r.session_date);
                        if (dt.getUTCDate() === d && dt.getUTCMonth() + 1 === reqMonth && dt.getUTCFullYear() === reqYear)
                            return true;
                        const istDt = new Date(dt.getTime() + IST_OFFSET_MS);
                        if (istDt.getUTCDate() === d && istDt.getUTCMonth() + 1 === reqMonth && istDt.getUTCFullYear() === reqYear)
                            return true;
                    }
                    if (r.created_at) {
                        const cd = new Date(r.created_at);
                        const istCd = new Date(cd.getTime() + IST_OFFSET_MS);
                        if (istCd.getUTCDate() === d && istCd.getUTCMonth() + 1 === reqMonth && istCd.getUTCFullYear() === reqYear)
                            return true;
                    }
                    return false;
                });
                ...
                for (const row of dayRows) {
                    ...
                    daySeconds += dur;
                    const isPos = row.outcome_status && DailyTracker_1.POSITIVE_OUTCOMES.includes(row.outcome_status);
                    if (isPos)
                        dayPositives++;
                    if (row.college_id) {
                        const cId = row.college_id._id?.toString() || row.college_id.toString();
        ...
        const startYear = parseInt(req.query.start_year, 10) || 2026;
        const startMonth = parseInt(req.query.start_month, 10) || 9;
        const monthsData = [];
        let prevMonthSeconds = null;
        ...
        for (let i = 0; i < 12; i++) {
            ...
            for (const r of rows) {
                ...
                mSecs += dur;
                if (r.outcome_status && DailyTracker_1.POSITIVE_OUTCOMES.includes(r.outcome_status))
                    mPos++;
                if (r.college_id) {
                    const name = r.college_id.college_name || 'Partner College';
...
app.get('/api/v1/dashboard/college-kpis', async (req, res) => {
    try {
        ...
        const kpiResults = await Promise.all(targetCollegeIds.map(async (cIdStr) => {
            ...
            const outcomeGroups = await DailyTracker_1.DailyTracker.aggregate([
            ...
            for (const g of outcomeGroups) {
                const bucket = g._id ? (OUTCOME_BUCKET[g._id] ?? 'other_progress') : 'pending';
...
app.get('/api/v1/dashboard/monthly-calls', async (req, res) => {
    try {
        const coordinatorId = (0, routePolicy_1.scopeToSelf)(req, req.query.coordinator_id);
        if (!coordinatorId || !mongoose_1.Types.ObjectId.isValid(coordinatorId)) {
        ...
        const coordinatorObjId = new mongoose_1.Types.ObjectId(coordinatorId);
        const today = getTodayDate();
        ...
        const ids = String(req.query.college_ids || '')
            ...
            .filter(Boolean)
            .slice(0, 10);
        const series = (await Promise.all(ids.map(async (idOrCode) => {
            let college = null;
            if (mongoose_1.Types.ObjectId.isValid(idOrCode))
                college = await College_1.College.findById(idOrCode);
            if (!college) {
            ...
            const dailyCalls = new Array(daysInMonth).fill(0);
            ...
            const shownBuckets = ['positive', 'not_hiring', 'negative', 'follow_up'];
            const dailyOutcomes = {
            ...
            const dailyDurationSecs = new Array(daysInMonth).fill(0);
            for (const row of rows) {
                let d = null;
                ...
                if (d && d >= 1 && d <= daysInMonth) {
                    dailyCalls[d - 1]++;
                    const bucket = OUTCOME_BUCKET[String(row.outcome_status || '')];
                    if (bucket && bucket in dailyOutcomes) {
                        dailyOutcomes[bucket][d - 1]++;
                    }
                    let durSec = 0;
                    if (typeof row.duration_seconds === 'number' && row.duration_seconds > 0) {
...
app.get('/api/v1/dashboard/team-leader', async (req, res) => {
    try {
        ...
        if (sujithaUser) {
            const sujithaColleges = await College_1.College.find({
                $or: [
        ...
        const targetLeader = (req.query.coordinator_id ? coordinators.find(c => String(c._id) === String(req.query.coordinator_id)) : null) ||
            sujithaUser ||
        ...
        const leaderHourlyCalls = new Array(24).fill(0);
        const IST_OFFSET_MS = 5.5 * 60 * 60 * 1000;
        const POSITIVE_OUTCOMES = ['invite_mail', 'positive', 'callback_requested', 'jd_received'];
        if (targetLeader) {
            const leaderRows = await DailyTracker_1.DailyTracker.find({
            ...
            for (const row of leaderRows) {
                ...
                if (row.outcome_status && POSITIVE_OUTCOMES.includes(row.outcome_status)) {
...
app.delete('/api/v1/metadata/:id', authMiddleware_1.authenticateJWT, (0, authMiddleware_1.authorizeRoles)('ADMINISTRATOR', 'ADMIN', 'TEAM_LEADER', 'TEAM_LEAD', 'PLACEMENT_COORDINATOR', 'COORDINATOR'), async (req, res) => {
    try {
        ...
        const user = req.user;
        const isCoordOnly = user?.roles?.some((r) => ['PLACEMENT_COORDINATOR', 'COORDINATOR'].includes(r)) &&
            !user?.roles?.some((r) => ['ADMINISTRATOR', 'ADMIN', 'TEAM_LEADER', 'TEAM_LEAD'].includes(r));
        if (isCoordOnly) {
            const isMohana = (user?.email || '').toLowerCase().includes('mohanaradha') ||
        ...
        record.deleted_by = req.user?.userId ? new mongoose_1.Types.ObjectId(req.user.userId) : null;
        await record.save();
        return res.status(200).json({
            success: true,
...
app.post('/api/v1/metadata/:id/restore', authMiddleware_1.authenticateJWT, (0, authMiddleware_1.authorizeRoles)('ADMINISTRATOR', 'ADMIN', 'TEAM_LEADER', 'TEAM_LEAD', 'PLACEMENT_COORDINATOR', 'COORDINATOR'), async (req, res) => {
    try {
        ...
        if ((0, routePolicy_1.refuseForeignOwner)(req, res, record.deleted_by ? String(record.deleted_by) : undefined, 'You can only restore records you deleted yourself.'))
            return;
...
app.delete('/api/v1/metadata/:id/purge', authMiddleware_1.authenticateJWT, (0, authMiddleware_1.authorizeRoles)('ADMINISTRATOR', 'ADMIN', 'TEAM_LEADER', 'TEAM_LEAD', 'PLACEMENT_COORDINATOR', 'COORDINATOR'), async (req, res) => {
    try {
    ...
    try {
        ...
        if (password && password.trim()) {
            ...
            if (!(0, passwordPolicy_1.isPasswordValid)(trimmedPassword)) {
                return res.status(400).json({
                    success: false,
                    error: { code: 'PASSWORD_POLICY', message: (0, passwordPolicy_1.firstPasswordError)(trimmedPassword) || 'Password does not meet the policy.' },
                });
            }
            const salt = await bcryptjs_1.default.genSalt(10);
...
app.get('/api/v1/settings', authMiddleware_1.authenticateJWT, async (req, res) => {
    try {
        ...
        const [recentCompanies, recentHrContacts, recentReports] = await Promise.all([
            CompanyMetadata_1.CompanyMetadata.countDocuments({ is_deleted: false, created_at: { $gte: sevenDaysAgo } }),
            CompanyMetadata_1.CompanyMetadata.countDocuments({
                is_deleted: false,
                created_at: { $gte: sevenDaysAgo },
                hr_name: { $exists: true, $ne: '' },
            }),
            ReportLibrary_1.ReportLibrary.countDocuments({ is_deleted: false, created_at: { $gte: sevenDaysAgo } }),
        ]);
        return res.status(200).json({
            success: true,
            data: {
...
app.all('/api/v1/weekly-tracker-import/sheet', async (req, res) => {
    try {
        ...
        for (const entry of parsedEntries) {
            ...
            const compMeta = await CompanyMetadata_1.CompanyMetadata.findOne({
                company_name: new RegExp(`^${escapeRegex(entry.companyName.trim())}$`, 'i'),
                is_deleted: false,
            });
            const resolvedCompId = compMeta?._id || new mongoose_1.Types.ObjectId();
            await WeeklyTracker_1.WeeklyTracker.create({
                academic_year: 2026,
                college_id: college._id,
                coordinator_id: coordinator?._id,
    ...
    try {
        ...
        if (mohanaDoc) {
            ...
            if (targetCollegeIds.length > 0) {
                ...
                const acetDoc = targetCollegeDocs.find((c) => c.college_code === 'ACET');
                if (acetDoc) {
                    await DailyTracker_1.DailyTracker.updateMany({ college_id: acetDoc._id }, { $set: { coordinator_id: mohanaDoc._id } });
                }
                const adminUsersList = await User_1.User.find({
                    role_codes: { $in: ['ADMINISTRATOR', 'ADMIN'] },
                }).select('_id');
    ...
    try {
        ...
        if (unnumberedCount > 0) {
            ...
            for (const doc of unnumberedDocs) {
                ...
                await CompanyMetadata_1.CompanyMetadata.updateOne({ _id: doc._id }, { $set: { serial_number: currentSerial } });
            }
...
const startServer = async () => {
    ...
    if (SEED_ON_BOOT) {
        ...
        await (0, updateNehruWeeklyTracker_1.updateNehruWeeklyTracker)();
        await (0, updateHitsWeeklyTracker_1.updateHitsWeeklyTracker)();
    }
    else {
    ...
    try {
        ...
        for (const jd of jdLeads) {
            if (!jd.lead_date || !jd.company_name)
                continue;

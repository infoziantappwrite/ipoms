...
function formatFollowUpMonth(val) {
    if (!val)
        return null;
...
function mapOutcome(status, comments) {
...
async function executeSeptemberIngestion() {
    ...
    for (const comp of NEW_COMPANIES_TO_ADD) {
        ...
        if (existing) {
            ...
            await CompanyMetadata_1.CompanyMetadata.updateOne({ _id: existing._id }, {
                $set: {
    ...
    for (const m of allMetadata) {
        ...
        if (exact) {
            if (!metaByExact.has(exact))
                metaByExact.set(exact, []);
    ...
    for (const sheetName of wb.SheetNames) {
        if (['Tracker', 'POSITIVES', 'JD RECEIVED'].includes(sheetName))
            continue;
        ...
        for (let r = headerIdx + 1; r < rawRows.length; r++) {
            ...
            if (isEmail(fieldPhone) && !email)
                email = fieldPhone;
            if (isEmail(fieldHr) && !email)
                email = fieldHr;
        ...
        if (matches.length > 0) {
            for (const metaDoc of matches) {
                ...
                if (isPlaceholder || isMissingInfo) {
                    ...
                    if (newHr && !metaDoc.hr_name)
                        updateObj.hr_name = newHr;
                    if (newPhone && !metaDoc.primary_mobile)
                        updateObj.primary_mobile = newPhone;
                    if (newEmail && !metaDoc.primary_email)
                        updateObj.primary_email = newEmail;
                    if (combinedMobiles.length > 0)
                        updateObj.mobile_numbers = combinedMobiles;
                    if (combinedEmails.length > 0)
                        updateObj.email_ids = combinedEmails;
                    await CompanyMetadata_1.CompanyMetadata.updateOne({ _id: metaDoc._id }, { $set: updateObj });
    ...
    console.log('───────────────────────────────────────────────────────────────');
    // Load all colleges
    const allColleges = await College_1.College.find({}).lean();
    ...
    const defaultAdmin = await User_1.User.findOne({}).sort({ created_at: 1 }).lean();
    const defaultUserId = defaultAdmin?._id || new mongoose_1.default.Types.ObjectId();
    // Reload metadata lookup
    const freshMetadata = await CompanyMetadata_1.CompanyMetadata.find({ is_deleted: false }).lean();
    ...
    const callsByCollegeAndDate = new Map();
    for (const call of parsedCalls) {
        const key = `${call.collegeName}__${call.sessionDate.toISOString().split('T')[0]}`;
        if (!callsByCollegeAndDate.has(key))
            callsByCollegeAndDate.set(key, []);
    ...
    for (const [batchKey, calls] of callsByCollegeAndDate.entries()) {
        ...
        if (!colDoc) {
            ...
            const newCol = await College_1.College.create({
        ...
        await DailyTracker_1.DailyTracker.deleteMany({
        ...
        for (const call of calls) {
            ...
            const companyId = metaDoc?._id || new mongoose_1.default.Types.ObjectId();
        ...
        if (docsToInsert.length > 0) {
            await DailyTracker_1.DailyTracker.insertMany(docsToInsert);

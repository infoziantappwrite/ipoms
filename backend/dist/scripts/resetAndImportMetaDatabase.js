...
const detectCompanyType = (companyName) => {
    ...
    if (name.includes('construction') || name.includes('builder') || name.includes('infra') || name.includes('estate') || name.includes('architect') || name.includes('housing'))
        return 'construction';
    if (name.includes('pharma') || name.includes('health') || name.includes('biotech') || name.includes('medical') || name.includes('hospital') || name.includes('clinic'))
        return 'pharma';
    if (name.includes('bank') || name.includes('finance') || name.includes('fintech') || name.includes('capital') || name.includes('invest') || name.includes('wealth'))
        return 'banking';
    if (name.includes('edtech') || name.includes('academy') || name.includes('learning') || name.includes('school') || name.includes('institute') || name.includes('education'))
        return 'edtech';
    if (name.includes('ai ') || name.includes('robotics') || name.includes('analytics') || name.includes('intelligence') || name.includes('agentic'))
        return 'ai';
    if (name.includes('auto') || name.includes('motor') || name.includes('engineer') || name.includes('steel') || name.includes('power') || name.includes('forge') || name.includes('mech') || name.includes('electric') || name.includes('appliances'))
        return 'core_engineering';
    if (name.includes('bpo') || name.includes('kpo') || name.includes('call center'))
        return 'bpo';
    if (name.includes('consulting') || name.includes('advisory') || name.includes('staffing') || name.includes('hr services') || name.includes('manpower'))
        return 'consulting';
    if (name.includes('product') || name.includes('technologies') || name.includes('labs') || name.includes('devices'))
        return 'product';
    if (name.includes('tech') || name.includes('soft') || name.includes('info') || name.includes('solution') || name.includes('digital') || name.includes('cloud') || name.includes('system') || name.includes('cyber') || name.includes('corp') || name.includes('infotech'))
        return 'software';
    return 'other';
};
async function resetAndImportMetaDatabase() {
    ...
    for (let i = 0; i < documentsToInsert.length; i += chunkSize) {
        ...
        await CompanyMetadata_1.CompanyMetadata.insertMany(chunk, { ordered: true });
    ...
    const firstDoc = await CompanyMetadata_1.CompanyMetadata.findOne({}).sort({ serial_number: 1 });
    const lastDoc = await CompanyMetadata_1.CompanyMetadata.findOne({}).sort({ serial_number: -1 });
    console.log('\n=============================================================');

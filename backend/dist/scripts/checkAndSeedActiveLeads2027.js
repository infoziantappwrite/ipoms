...
async function checkAndSeed() {
    try {
        ...
        const existingLeads = await ActiveLead_1.ActiveLead.find({ academic_year: '2027', is_deleted: false });
        ...
        for (const lead of NEW_LEADS_BATCH) {
            ...
            const found = existingLeads.find((ex) => normalizeComp(ex.company_name) === normName || ex.company_name.toLowerCase() === lead.company_name.toLowerCase());
            if (found) {
        ...
        const finalTotal = await ActiveLead_1.ActiveLead.countDocuments({ academic_year: '2027', is_deleted: false });
        console.log(`\n========================================`);

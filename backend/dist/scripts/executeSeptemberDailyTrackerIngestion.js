...
exports.ingestAllCollegeTrackers = ingestAllCollegeTrackers;
...
function normalizeFollowUpMonth(raw, outcome) {
...
function normalizeCompanyType(raw) {
    ...
    if (VALID_COMPANY_TYPES.has(cleaned))
        return cleaned;
    if (cleaned.includes('software') || cleaned.includes('it'))
        return 'software';
    if (cleaned.includes('bank'))
        return 'banking';
    if (cleaned.includes('finance'))
        return 'finance';
    if (cleaned.includes('construct'))
        return 'construction';
    if (cleaned.includes('core') || cleaned.includes('engineer') || cleaned.includes('automotive'))
        return 'core_engineering';
    if (cleaned.includes('edu'))
        return 'edtech';
    if (cleaned.includes('health') || cleaned.includes('pharma') || cleaned.includes('medic'))
        return 'pharma';
    if (cleaned.includes('consult'))
        return 'consulting';
    if (cleaned.includes('bpo'))
        return 'bpo';
    if (cleaned.includes('product'))
        return 'product';
    return 'other';
}
async function ingestAllCollegeTrackers() {
    ...
    const allUsers = await User_1.User.find({ is_active: { $ne: false } }).lean();
    ...
    for (let i = 0; i < wb.SheetNames.length; i++) {
        ...
        if (!coordinatorUser) {
            ...
            coordinatorUser = allUsers.find((u) => !u.role_codes?.includes('ADMINISTRATOR') &&
                (u.assigned_college_ids || []).some((id) => String(id) === String(college._id)));
        }
        if (!coordinatorUser) {

...
exports.syncAllPastDailyToWeekly = syncAllPastDailyToWeekly;
...
async function syncAllPastDailyToWeekly() {
    console.log('\n===============================================================');
    console.log('🔄 SYNCING ALL PAST DAILY TRACKER ENTRIES ➔ WEEKLY TRACKER');
    console.log('===============================================================\n');
    await (0, database_1.connectDatabase)();
    const targetYear = await (0, academicYear_1.getCurrentAcademicYear)();
    ...
    const colleges = await College_1.College.find({ is_active: { $ne: false } }).sort({ college_code: 1 });
    ...
    for (const college of colleges) {
        ...
        const existingWeekly = await WeeklyTracker_1.WeeklyTracker.find({
            college_id: { $in: queryCollegeIds },
        ...
        existingWeekly.forEach((w) => {
            if (w.company_name)
                seenNames.add(w.company_name.trim().toLowerCase());
        });
        ...
        for (const dRow of positiveDailyRows) {
            if (!dRow.company_name || !dRow.company_name.trim())
                continue;
            ...
            if (!seenNames.has(normalizedKey)) {
                ...
                let metaContacts = {};
                if (dRow.company_id && mongoose_1.Types.ObjectId.isValid(String(dRow.company_id))) {
                    const meta = await CompanyMetadata_1.CompanyMetadata.findById(dRow.company_id).lean();
                    if (meta) {
                        metaContacts = {
                ...
                await WeeklyTracker_1.WeeklyTracker.create({
                    ...
                    coordinator_id: dRow.coordinator_id || new mongoose_1.Types.ObjectId(),
    ...
    const updateResult = await WeeklyTracker_1.WeeklyTracker.updateMany({

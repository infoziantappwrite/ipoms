...
async function audit() {
    ...
    const activeLeads = await ActiveLead_1.ActiveLead.find({}).lean();
    ...
    for (const al of activeLeads) {
        if (al.company_name) {
            companyEntries.push({
                ...
                section: al.status || al.pipeline_section || 'N/A'
            });
        }
    }
    for (const mp of seedMasterDailyLeads_1.MASTER_POSITIVES_DATA) {
        if (mp.company) {
            companyEntries.push({
    ...
    const noSpaceMap = new Map();
    for (const entry of companyEntries) {
        const key = normalizeNoSpace(entry.company_name);
        if (!key)
            continue;
        if (!noSpaceMap.has(key))
            noSpaceMap.set(key, []);
    ...
    for (let i = 0; i < distinctBases.length; i++) {
        for (let j = i + 1; j < distinctBases.length; j++) {
            ...
            if (Math.abs(b1.length - b2.length) > 2)
                continue;

...
function parseSheet(sheet) {
    ...
    for (let r = 0; r < rawRows.length; r++) {
        ...
        if (currentSection) {
            ...
            if (['status', 'count', 'total', 's.no', 'si.no', 'company name', 'role', 'ctc', 'in progress', 'pipeline', 'completed', 'top companies'].includes(lowerComp) ||
...
async function run() {
    const targetSheet = process.argv[2] || 'KIOT';
    console.log(`\n======================================================`);
    console.log(`🚀 Ingesting Weekly Tracker for Sheet: "${targetSheet}"`);
    console.log(`======================================================\n`);
    ...
    if (!matchedSheetKey) {
        console.error(`❌ Sheet "${targetSheet}" not found in workbook. Available sheets:`, wb.SheetNames);
        process.exit(1);
    }
    ...
    let college = await mongoose.connection.collection('colleges').findOne({
        $or: [
    ...
    if (!college) {
        ...
        if (mappedCode) {
            college = await mongoose.connection.collection('colleges').findOne({
    ...
    console.log(`🏛️ Found College: "${college.college_name}" (${college.college_code}), ID: ${college._id}`);
    // Find assigned coordinator
    let coordinator = null;
    if (college.assigned_coordinator_ids && college.assigned_coordinator_ids.length > 0) {
        // Prefer non-admin coordinator if available
    ...
    console.log(`👤 Assigned Coordinator: ${coordinator.full_name || coordinator.name || coordinator.username} (${coordinator.email || coordinator.username}), ID: ${coordinator._id}`);
    // Delete existing records for this college and academic year 2027 to ensure clean replacement
    const delRes = await mongoose.connection.collection('weekly_tracker').deleteMany({
        college_id: college._id,
        academic_year: 2027,
    });
    console.log(`🗑️ Removed ${delRes.deletedCount} existing 2027 records for ${college.college_name}.`);
    const breakdown = {};
    const insertedRows = [];
    for (let idx = 0; idx < entries.length; idx++) {
        const entry = entries[idx];
        // Ensure CompanyMetadata
    ...
    console.log(`\n🎉 Successfully inserted ${insertedRows.length} records into Weekly Tracker for ${college.college_name}!`);
    console.log('\n--- Breakdown by Section ---');
    console.table(breakdown);
    console.log('\n--- Inserted Records ---');
    insertedRows.forEach((r, i) => {
        console.log(`${i + 1}. [${r.pipeline_section.toUpperCase()}] ${r.company_name} | Role: ${r.job_role} | CTC: ${r.ctc_lpa || 'N/A'} | Status: ${r.current_status_text}`);
    });

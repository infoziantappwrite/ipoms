...
exports.COLLEGE_DEFAULT_LOCATIONS = exports.SHAREPOINT_EXCEL_URL = void 0;
exports.parseDepartments = parseDepartments;
exports.parseSheetRowsWithLinks = parseSheetRowsWithLinks;
exports.cleanValue = cleanValue;
exports.findLatestExcelFile = findLatestExcelFile;
exports.syncCollegesFromExcel = syncCollegesFromExcel;
const mongoose_1 = __importDefault(require("mongoose"));
const XLSX = __importStar(require("xlsx"));
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const College_1 = require("../models/College");
exports.SHAREPOINT_EXCEL_URL = 'https://infoziant-my.sharepoint.com/:x:/p/mohanaradha_a/IQDG6kI3OOzoSqDFzVKw-Z1aAUdwEZZ92lkYdc04tgKwmnQ?download=1';
exports.COLLEGE_DEFAULT_LOCATIONS = {
    ACET: 'Puducherry',
...
function parseDepartments(deptStr) {
    if (!deptStr || typeof deptStr !== 'string')
        return [];
    ...
    for (const line of lines) {
        ...
        if (cleaned.toLowerCase().includes('artificial intelligence and data science') ||
...
function extractCellData(sheet, r, c) {
...
function parseSheetRowsWithLinks(sheet) {
    if (!sheet || !sheet['!ref'])
        return [];
...
async function syncCollegesFromExcel(bufferOrPath) {
    let workbook;
    if (bufferOrPath) {
        ...
        try {
            console.log('Downloading live Excel from SharePoint via fetch:', exports.SHAREPOINT_EXCEL_URL);
            const response = await fetch(exports.SHAREPOINT_EXCEL_URL, {
                headers: {
            ...
            if (response.ok) {
                ...
                if (buf.length > 500 && buf[0] === 0x50 && buf[1] === 0x4B) {
                    ...
                    try {
                        ...
                        fs_1.default.writeFileSync(path_1.default.join(rootDir, 'Colleges_and_Coordinators.xlsx'), buf);
                        fs_1.default.writeFileSync(path_1.default.join(rootDir, 'Colleges and Coordinators.xlsx'), buf);
                    }
                    catch (writeErr) {
    ...
    for (const sheetName of workbook.SheetNames) {
        ...
        for (const row of richRows) {
            ...
            if (!location && rawCode && exports.COLLEGE_DEFAULT_LOCATIONS[rawCode]) {
                location = exports.COLLEGE_DEFAULT_LOCATIONS[rawCode];
            }
            ...
            if (rawName && (!college.college_name || isNew))
                college.college_name = rawName;
            if (rawCode && (!college.college_code || isNew))
                college.college_code = rawCode;
            if (tpoName)
                college.tpo_name = tpoName;
            if (tpoEmail)
                college.tpo_email = tpoEmail;
            if (tpoPhone)
                college.tpo_contact_mobile = tpoPhone;
            if (nirf)
                college.nirf_ranking = nirf;
            if (highestPkg)
                college.highest_package_lpa = highestPkg;
            if (lowestPkg)
                college.lowest_package_lpa = lowestPkg;
            if (website)
                college.college_website = website;
            if (address) {
            ...
            if (inauguratedAt)
                college.established_year = inauguratedAt;
            // Clean up any old #VALUE! in DB
            if (location) {
                college.location = location;
            }
            else if (!college.location || college.location.startsWith('#')) {
                college.location = (rawCode && exports.COLLEGE_DEFAULT_LOCATIONS[rawCode]) || 'Tamil Nadu, India';
            }
            if (parsedDepts.length > 0)
                college.departments = parsedDepts;

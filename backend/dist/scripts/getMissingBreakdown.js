...
async function getStats() {
    ...
    const placeholdersMissing = missing.filter((c) => (c.serial_number || 0) >= 3807 && (c.serial_number || 0) <= 3998);
    const coreMissing = missing.filter((c) => (c.serial_number || 0) < 3807);
    const newMissing = missing.filter((c) => (c.serial_number || 0) > 3998);
    ...
    console.log(JSON.stringify(coreMissing.map((c) => ({
        sno: c.serial_number,
        name: c.company_name,
        hr: c.hr_name || '—',
    ...
    console.log(JSON.stringify(placeholdersMissing.map((c) => ({
        sno: c.serial_number,
        name: c.company_name,
        hr: c.hr_name || '—',
        email: c.primary_email || '—'

...
exports.importSingleCollege = importSingleCollege;
...
async function importSingleCollege(sheetName, options = {}) {
    await (0, database_1.connectDatabase)();
    if (options.clearAllFirst) {
        console.log('🧹 [Wipe] Clearing all existing WeeklyTracker records across all colleges...');
        const delResult = await WeeklyTracker_1.WeeklyTracker.deleteMany({});
    ...
    if (!options.clearAllFirst) {
        const delCount = await WeeklyTracker_1.WeeklyTracker.deleteMany({ college_id: college._id });

...
async function runDeepAnalysis() {
    ...
    for (const m of allMetadata) {
        if (m.serial_number)
            metaBySno.set(m.serial_number, m);
    ...
    for (const sheetName of wb.SheetNames) {
        const isCollegeSheet = COLLEGE_SHEET_NAMES.some((c) => c.trim().toLowerCase() === sheetName.trim().toLowerCase());
        if (!isCollegeSheet)
            continue;
        ...
        for (let r = 0; r < Math.min(5, rawRows.length); r++) {
            ...
            if (rowStr.includes('company') || rowStr.includes('contact') || rowStr.includes('mail')) {
                ...
                row.forEach((cell, cIdx) => {
                    ...
                    if (cName.includes('company'))
                        colMap.company = cIdx;
                    else if (cName.includes('date') || cName.includes('callingdate'))
                        colMap.date = cIdx;
                    else if (cName.includes('time'))
                        colMap.time = cIdx;
                    else if (cName.includes('hr') || cName.includes('person') || cName.includes('spoc'))
                        colMap.hr = cIdx;
                    else if (cName.includes('contact') || cName.includes('phone') || cName.includes('mobile') || cName.includes('num'))
                        colMap.phone = cIdx;
                    else if (cName.includes('mail') || cName.includes('email'))
                        colMap.email = cIdx;
                    else if (cName.includes('status') || cName.includes('response') || cName.includes('outcome'))
                        colMap.status = cIdx;
                    else if (cName.includes('follow') || cName.includes('month'))
                        colMap.followup = cIdx;
                    else if (cName.includes('comment') || cName.includes('remark') || cName.includes('feedback'))
        ...
        for (let r = headerRowIdx + 1; r < rawRows.length; r++) {
            ...
            if (!companyName && !fieldA && !fieldB && !email)
                continue;
            if (companyName.toLowerCase().includes('handled by') || companyName.toLowerCase().includes('company name'))
                continue;
            ...
            if (isEmail(fieldA) && !email)
                email = fieldA;
            if (isEmail(fieldB) && !email)
                email = fieldB;
    ...
    for (const item of extractedList) {
        ...
        if (item.hrName && item.hrName !== '—' && item.hrName !== '-')
            c.hrNames.add(item.hrName);
    ...
    const placeholderEnrichments = [];
    const completeMatches = [];
    const notInMetadataList = [];
    ...
    const outPath = path_1.default.resolve(__dirname, '../../../scratch/september_deep_audit.json');
    fs_1.default.writeFileSync(outPath, JSON.stringify({
        stats: {

...
async function runAudit() {
    ...
    for (const sheetName of wb.SheetNames) {
        ...
        for (let r = headerIdx + 1; r < rawRows.length; r++) {
            ...
            if (firstNonEmpty.toLowerCase().includes('september') ||
    ...
    for (const call of allCalls) {
        ...
        if (call.hrName && call.hrName !== '—' && call.hrName !== '-')
            agg.hrNames.add(call.hrName);
    ...
    fs_1.default.writeFileSync(path_1.default.join(outDir, 'september_tracker_calls.json'), JSON.stringify(allCalls, null, 2));
    fs_1.default.writeFileSync(path_1.default.join(outDir, 'september_placeholder_enrichments.json'), JSON.stringify(placeholderEnrichments, null, 2));
    fs_1.default.writeFileSync(path_1.default.join(outDir, 'september_not_in_metadata.json'), JSON.stringify(notInMetadata, null, 2));

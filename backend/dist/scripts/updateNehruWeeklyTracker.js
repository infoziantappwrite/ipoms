...
async function updateNehruWeeklyTracker() {
    ...
    if (!coordinator) {
        coordinator = await User_1.User.findOne({ role_codes: 'PLACEMENT_COORDINATOR' }) || await User_1.User.findOne();
    }
    // 3. Remove ALL existing data present for Nehru College in WeeklyTracker
    const deletedResult = await WeeklyTracker_1.WeeklyTracker.deleteMany({

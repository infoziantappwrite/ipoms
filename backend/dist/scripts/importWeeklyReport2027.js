...
exports.METADATA_TAG = exports.SHEET_TO_COLLEGE = void 0;
...
function parseSheet(ws) {
    ...
    for (const r of raw) {
        ...
        Object.keys(colmap).forEach((idxStr) => {
            ...
            if (v !== '' && v !== undefined)
                data[colmap[Number(idxStr)]] = v;
        });
        const name = data.company_name;
        if (!name || /^\d+$/.test(name))
            continue;
        if (/^(status|count|total|status count summary|s\.?no)$/i.test(name))
            continue;
...
exports.METADATA_TAG = '[Weekly Import 2027] auto-created placeholder - contact details pending';
...
function buildRow(r, collegeId, coordinatorId, companyId) {
    const d = r.data;
    return {
        academic_year: 2026,
...
async function main() {
    ...
    const allMeta = await CompanyMetadata_1.CompanyMetadata.find({ is_deleted: false }).select('_id company_name');
    const canonIndex = new Map();
    for (const m of allMeta) {
        const k = canon(m.company_name);
        if (k && !canonIndex.has(k))
            canonIndex.set(k, m._id);
    }
    ...
    for (const sheet of wb.SheetNames) {
        if (sheet.toUpperCase() === 'PENDING')
            continue;
        const re = exports.SHEET_TO_COLLEGE[sheet.trim()];
        ...
        const existing = await WeeklyTracker_1.WeeklyTracker.countDocuments({ college_id: college._id });
        const coordDoc = await WeeklyTracker_1.WeeklyTracker.findOne({ college_id: college._id }).select('coordinator_id');
        let coordinatorId = coordDoc ? coordDoc.coordinator_id : null;
        let coordNote = '';
        if (!coordinatorId) {
            // no existing rows to inherit from - fall back to a coordinator assigned to this college
            const assigned = await User_1.User.findOne({
                role_codes: 'PLACEMENT_COORDINATOR',
                account_status: 'active',
                assigned_college_ids: college._id,
            }).select('_id full_name');
            const anyCoord = assigned || await User_1.User.findOne({
        ...
        for (const r of rows) {
            ...
            let companyId = null;
            const hit = await CompanyMetadata_1.CompanyMetadata.findOne({
        ...
        if (apply) {
            const del = await WeeklyTracker_1.WeeklyTracker.deleteMany({ college_id: college._id });
            if (toInsert.length)
                await WeeklyTracker_1.WeeklyTracker.insertMany(toInsert, { ordered: false });
        ...
        if (unresolved.length)
            unresolvedAll.push([sheet, unresolved]);
        report.push('  ' + sheet.padEnd(14) + String(existing).padStart(4) + ' existing -> ' +
    ...
    if (unresolvedAll.length) {
        ...
        for (const pair of unresolvedAll) {
            ...
            if (pair[1].length > 5)
                console.log('      ... and ' + (pair[1].length - 5) + ' more');
        }
    }
    // ---- Excel report of companies that were not in the metadata database ----
    const perRow = [];
    for (const pair of unresolvedDetail)
        perRow.push(pair);
    const seen = new Map();
    for (const row of perRow) {
        const k = canon(row['Company Name']);
        if (!seen.has(k))
            seen.set(k, { 'Company Name': row['Company Name'], Colleges: row.College, Occurrences: 1 });
        else {
            const e = seen.get(k);
            e.Occurrences++;
            if (String(e.Colleges).indexOf(row.College) === -1)
                e.Colleges += ', ' + row.College;
        }
    }

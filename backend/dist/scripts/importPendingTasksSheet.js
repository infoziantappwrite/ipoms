...
exports.importPendingTasks = importPendingTasks;
...
const FILE_PATH = candidatePaths.find(p => fs_1.default.existsSync(p)) || candidatePaths[0];
...
async function importPendingTasks() {
    ...
    const delResult = await PendingTask_1.PendingTask.deleteMany({});
    ...
    for (let r = 0; r < rawRows.length; r++) {
        ...
        if (detectedCollegeKey) {
            ...
            currentCollege = await College_1.College.findOne({
                $or: [
        ...
        if (currentCollege && currentCoordinator && headerMap['company'] !== undefined) {
            ...
            if (!compMeta) {
                currentSerial += 1;
                const isTech = companyVal.toLowerCase().includes('technolog') ||

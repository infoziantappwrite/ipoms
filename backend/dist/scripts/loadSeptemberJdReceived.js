...
async function loadSeptemberJdReceived() {
    ...
    try {
        ...
        const coordinator = (await User_1.User.findOne({ account_status: 'active', is_deleted: { $ne: true }, role_codes: 'PLACEMENT_COORDINATOR' })) ||
            (await User_1.User.findOne({ account_status: 'active', is_deleted: { $ne: true } })) ||
            (await User_1.User.findOne({ is_deleted: { $ne: true } }));
        const coordinatorId = coordinator ? coordinator._id : new mongoose_1.Types.ObjectId();
        ...
        for (const [code, meta] of Object.entries(seedAugustAllCollegesJdReceived_1.COLLEGE_META_MAP)) {
            ...
            collegeIdMap.set(code.toUpperCase(), college._id);
            for (const alias of meta.aliases) {
                collegeIdMap.set(alias.toUpperCase(), college._id);
            }
        }
        ...
        for (const item of SEPTEMBER_3_AND_4_JD_DATA) {
            ...
            if (!meta) {
                const lastMeta = await CompanyMetadata_1.CompanyMetadata.findOne().sort({ serial_number: -1 }).select('serial_number');
                const nextSNo = (lastMeta?.serial_number || 0) + 1;
                meta = await CompanyMetadata_1.CompanyMetadata.create({

...
exports.fillCleanMetadataPlaceholders = fillCleanMetadataPlaceholders;
const CompanyMetadata_1 = require("../models/CompanyMetadata");
const database_1 = require("../config/database");
const CLEAN_13_MATCHES = [
    {
        targetSno: 3809,
...
async function fillCleanMetadataPlaceholders(dryRun = false) {
    console.log('\n===============================================================');
    ...
    for (const pair of CLEAN_13_MATCHES) {
        const target = await CompanyMetadata_1.CompanyMetadata.findOne({
            serial_number: pair.targetSno,
            is_deleted: false,
        });
        const source = await CompanyMetadata_1.CompanyMetadata.findOne({
        ...
        if (hr_name)
            updatePayload.hr_name = hr_name;
        if (source?.hr_designation)
            updatePayload.hr_designation = source.hr_designation;
        if (primary_mobile)
            updatePayload.primary_mobile = primary_mobile;
        if (mobile_numbers.length > 0)
            updatePayload.mobile_numbers = mobile_numbers;
        if (primary_email)
            updatePayload.primary_email = primary_email;
        if (email_ids.length > 0)
            updatePayload.email_ids = email_ids;
        if (source?.company_type && !target.company_type)
            updatePayload.company_type = source.company_type;
        if (source?.industry_sector && !target.industry_sector)
            updatePayload.industry_sector = source.industry_sector;
        ...
        if (!dryRun) {
            await CompanyMetadata_1.CompanyMetadata.updateOne({ _id: target._id }, { $set: updatePayload });

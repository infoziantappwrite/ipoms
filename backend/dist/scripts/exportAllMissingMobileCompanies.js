...
async function generateAllMissingMobileExcel() {
    ...
    const allMissing = await CompanyMetadata_1.CompanyMetadata.find(emptyFilter)
        .sort({ serial_number: 1 })
        .lean();
    console.log(`📋 Found ${allMissing.length} total records missing mobile numbers.`);
    const category1Legacy = allMissing.filter((c) => (c.hr_name && c.hr_name.trim()) || (c.primary_email && c.primary_email.trim()));
    const category2Placeholders = allMissing.filter((c) => !(c.hr_name && c.hr_name.trim()) && !(c.primary_email && c.primary_email.trim()));
    ...
    function formatSheet(sheet, data, headerBgHex) {
        ...
        data.forEach((comp, idx) => {
            ...
            const row = sheet.addRow({
                ...
                alt_mobiles: (comp.mobile_numbers || []).filter((m) => m !== comp.primary_mobile).join(', '),
                primary_email: comp.primary_email || '',
                alt_emails: (comp.email_ids || []).filter((e) => e !== comp.primary_email).join(', '),
    ...
    // Output Paths
    const primaryPath = path_1.default.resolve(__dirname, '../../../442_Missing_Mobile_Metadata_Companies.xlsx');

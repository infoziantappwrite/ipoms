...
exports.seedDefaultChatChannels = seedDefaultChatChannels;
...
function broadcastChatEvent(event, data, targetUserIds) {
...
async function seedDefaultChatChannels() {
    try {
        // Strictly enforce ONLY 1 Team Channel: Placements (delete urgent-doubts and any other channels)
        await ChatConversation_1.ChatConversation.deleteMany({
            type: 'channel',
            channel_slug: { $ne: 'placements' },
        }).catch(() => { });
        // Ensure Placements channel exists
        const placementsChannel = await ChatConversation_1.ChatConversation.findOne({
            type: 'channel',
        ...
        if (placementsChannel) {
            ...
            await ChatConversation_1.ChatConversation.create({
                type: 'channel',
                title: 'Placements',
                channel_slug: 'placements',
...
function registerChatRoutes(app) {
    ...
    app.get('/api/v1/chat/conversations', async (req, res) => {
        try {
            ...
            const filter = {
                ...
                $or: [
                    { type: 'channel' },
                    ...(currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)
                        ? [{ type: 'direct', participant_ids: new mongoose_1.default.Types.ObjectId(currentUserId) }]
                        : []),
                ],
            };
            ...
            const conversations = await ChatConversation_1.ChatConversation.find(filter)
            ...
            const enriched = await Promise.all(conversations.map(async (conv) => {
                ...
                if (currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)) {
                    unreadCount = await ChatMessage_1.ChatMessage.countDocuments({
                        conversation_id: conv._id,
                        sender_id: { $ne: new mongoose_1.default.Types.ObjectId(currentUserId) },
                        read_by: { $nin: [new mongoose_1.default.Types.ObjectId(currentUserId)] },
                    });
                }
                let dynamicTitle = conv.title;
                if (conv.type === 'direct' && Array.isArray(conv.participant_ids)) {
                    const otherParticipant = conv.participant_ids.find((p) => String(p?._id || p) !== String(currentUserId));
                    if (otherParticipant && otherParticipant.full_name) {
                        dynamicTitle = otherParticipant.full_name;
                    }
                }
                return {
                    ...conv,
            ...
            const validConversations = enriched.filter((conv) => {
                if (conv.type === 'direct') {
                    const others = (conv.participant_ids || []).filter((p) => String(p?._id || p) !== String(currentUserId));
    ...
    app.get('/api/v1/chat/conversations/:id/messages', async (req, res) => {
        try {
            ...
            const conversation = await ChatConversation_1.ChatConversation.findById(id).lean();
            if (!conversation) {
            ...
            if (conversation.type === 'direct') {
                const isParticipant = currentUserId &&
                    conversation.participant_ids?.some((p) => String(p) === String(currentUserId) || String(p._id) === String(currentUserId));
                if (!isParticipant) {
            ...
            if (currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)) {
                msgFilter.deleted_for_users = { $nin: [new mongoose_1.default.Types.ObjectId(currentUserId)] };
            }
            const messages = await ChatMessage_1.ChatMessage.find(msgFilter)
                .sort({ created_at: 1 })
                .limit(limit)
                .lean();
            // Automatically mark messages as read by current user and broadcast read event
            if (currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)) {
                const userObjId = new mongoose_1.default.Types.ObjectId(currentUserId);
                ChatMessage_1.ChatMessage.updateMany({ conversation_id: id, read_by: { $ne: userObjId } }, { $addToSet: { read_by: userObjId } }).then((result) => {
    ...
    app.post('/api/v1/chat/conversations', async (req, res) => {
        try {
            ...
            if (type === 'direct') {
                if (!recipient_id || !currentUserId || !mongoose_1.default.Types.ObjectId.isValid(recipient_id) || !mongoose_1.default.Types.ObjectId.isValid(currentUserId)) {
                ...
                const existing = await ChatConversation_1.ChatConversation.findOne({
                    type: 'direct',
                    participant_ids: {
                        $all: [new mongoose_1.default.Types.ObjectId(currentUserId), new mongoose_1.default.Types.ObjectId(recipient_id)],
                    },
                })
                    .populate('participant_ids', 'full_name username official_email profile_photo_url role_codes')
                    .lean();
                ...
                const recipient = await User_1.User.findById(recipient_id);
                const newDirect = await ChatConversation_1.ChatConversation.create({
                    type: 'direct',
                    title: recipient ? recipient.full_name : 'Direct Chat',
                    participant_ids: [
                        new mongoose_1.default.Types.ObjectId(currentUserId),
                        new mongoose_1.default.Types.ObjectId(recipient_id),
                    ],
                    created_by: new mongoose_1.default.Types.ObjectId(currentUserId),
                });
                const populated = await ChatConversation_1.ChatConversation.findById(newDirect._id)
                    .populate('participant_ids', 'full_name username official_email profile_photo_url role_codes')
                    .lean();
                ...
                const newChannel = await ChatConversation_1.ChatConversation.create({
                    type: 'channel',
                    title,
                    channel_slug: channelSlug,
                    description: description || '',
                    created_by: currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId) ? new mongoose_1.default.Types.ObjectId(currentUserId) : undefined,
                });
    ...
    app.post('/api/v1/chat/conversations/:id/messages', async (req, res) => {
        try {
            const { id } = req.params;
            const currentUserId = getAuthUserId(req);
            const { message_text, message_type = 'text', metadata = {}, reply_to, } = req.body;
            ...
            let senderName = req.body.sender_name || req.user?.fullName || '';
            ...
            if (!senderName) {
                senderName = req.body.sender_name || req.user?.fullName || 'Coordinator';
            }
            const newMsg = await ChatMessage_1.ChatMessage.create({
                conversation_id: id,
                sender_id: (currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId))
                    ? new mongoose_1.default.Types.ObjectId(currentUserId)
                ...
                read_by: currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)
                    ? [new mongoose_1.default.Types.ObjectId(currentUserId)]
                    : [],
            });
            // Update conversation last message timestamp & snippet
            await ChatConversation_1.ChatConversation.findByIdAndUpdate(id, {
                last_message_at: new Date(),
            ...
            const targetUsers = conversation.type === 'direct'
                ? conversation.participant_ids.map((p) => String(p))
                : undefined;
    ...
    app.post('/api/v1/chat/messages/:id/reactions', async (req, res) => {
        try {
            const { id } = req.params;
            ...
            const msg = await ChatMessage_1.ChatMessage.findById(id);
            if (!msg) {
                return res.status(404).json({ success: false, error: { message: 'Message not found' } });
            }
            const user = currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)
                ? await User_1.User.findById(currentUserId)
                : null;
            const userName = user?.full_name || req.body.user_name || 'Coordinator';
            const existingIndex = (msg.reactions || []).findIndex((r) => String(r.user_id) === String(currentUserId) && r.emoji === emoji);
            if (existingIndex > -1) {
                ...
                msg.reactions.push({
                    emoji,
                    user_id: currentUserId ? new mongoose_1.default.Types.ObjectId(currentUserId) : new mongoose_1.default.Types.ObjectId(),
                    user_name: userName,
                });
            }
            await msg.save();
    ...
    app.post('/api/v1/chat/conversations/:id/read', async (req, res) => {
        try {
            ...
            if (currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)) {
                const userObjId = new mongoose_1.default.Types.ObjectId(currentUserId);
                const updateRes = await ChatMessage_1.ChatMessage.updateMany({ conversation_id: id, read_by: { $ne: userObjId } }, { $addToSet: { read_by: userObjId } });
    ...
    app.get('/api/v1/chat/coordinators', async (req, res) => {
        try {
            const currentUserId = getAuthUserId(req);
            const filter = {
                is_deleted: { $ne: true },
    ...
    app.delete('/api/v1/chat/messages/:id', async (req, res) => {
        try {
            ...
            const user = currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)
                ? await User_1.User.findById(currentUserId)
                : null;
            const isAdmin = user?.role_codes?.some((r) => {
            ...
            if (deleteType === 'everyone') {
                ...
                await ChatMessage_1.ChatMessage.findByIdAndDelete(id);
                // Update latest message in conversation
                const latest = await ChatMessage_1.ChatMessage.findOne({ conversation_id: conversationId, is_deleted: false })
                    .sort({ created_at: -1 })
                    .lean();
                ...
                if (currentUserId && mongoose_1.default.Types.ObjectId.isValid(currentUserId)) {
                    const userObjId = new mongoose_1.default.Types.ObjectId(currentUserId);
                    await ChatMessage_1.ChatMessage.findByIdAndUpdate(id, {
    ...
    app.delete('/api/v1/chat/conversations/:id', async (req, res) => {
        try {
            const { id } = req.params;
            const conv = await ChatConversation_1.ChatConversation.findById(id);
            if (!conv) {
                return res.status(404).json({ success: false, error: { message: 'Conversation not found' } });
            }
            // Delete all messages in this conversation
            await ChatMessage_1.ChatMessage.deleteMany({ conversation_id: id });
            if (conv.type === 'direct') {
                await ChatConversation_1.ChatConversation.findByIdAndDelete(id);
            }
            else {
                await ChatConversation_1.ChatConversation.findByIdAndUpdate(id, {

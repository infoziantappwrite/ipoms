...
**Placement Coordinator (daily user):**
1. Log in with email + password
2. Land on the Dashboard — greeting, notifications, today's tasks
3. Work the Daily Tracker — log outreach calls, start/end time, auto-saves as you go
4. Move promising calls into Daily Leads (Positives → JD Received)
5. Once a day finalizes (23:59:59 IST), that data feeds into the Weekly Tracker pipeline
6. Generate Reports from the Weekly Tracker when needed
7. Use Team Chat alongside all of this for doubts and coordination with the team

**Team Leader / Administrator:**
- Assigns colleges to coordinators, which is what shows up as their work on the Dashboard
- Every report generated writes an entry to an immutable audit log

**TPO (external, read-only):**
- Only sees the finalized Weekly Placement Report for their own college — nothing else
...
Everything hangs off `users` and `colleges`. A coordinator is assigned to colleges; their daily calls go into `daily_tracker`, which rolls up into `weekly_tracker` (the placement pipeline) and `daily_leads`. `company_metadata` is a shared, standalone company/HR-contact directory — not owned by any one record, just referenced by name. Reports and the chat system reference users and colleges too, and every report generation writes to an immutable `audit_logs` collection.
